...
public:
    ...
    void on_bar(const Bar& bar) override {
        ...
        sig = (is_first_tick_ ? _ta_crossover_3.compute(emaFast, emaSlow) : _ta_crossover_3.recompute(emaFast, emaSlow));
        exitSig = (is_first_tick_ ? _ta_crossunder_4.compute(emaFast, emaSlow) : _ta_crossunder_4.recompute(emaFast, emaSlow));
        if ((sig && (signed_position_size() == 0))) {
            lvl = current_bar_.close;
            strategy_entry(std::string("L"), true, (lvl * 0.98), na<double>(), na<double>(), "", "", 0, -1);
            strategy_exit(std::string("X"), std::string("L"), (lvl * 1.04), (lvl * 0.965), na<double>(), na<double>(), na<double>(), 100.0, "", na<double>(), "");
        }
        if ((exitSig && (signed_position_size() > 0))) {
            strategy_close(std::string("L"), std::string("opp"), na<double>(), na<double>(), false);
        }
    }
