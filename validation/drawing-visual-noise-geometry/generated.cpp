...
public:
    ...
    void on_bar(const Bar& bar) override {
        ...
        sma = (is_first_tick_ ? _ta_sma_1.compute(current_bar_.close) : _ta_sma_1.recompute(current_bar_.close));
        if (is_na(lv)) {
            lv = pf_line_new(_pf_lines_, (int64_t)(bar_index_), (double)(sma), (int64_t)((bar_index_ + 1)), (double)(sma), XLoc::bar_index, true, true);
            lv2 = pf_line_new(_pf_lines_, (int64_t)(bar_index_), (double)(sma), (int64_t)((bar_index_ + 1)), (double)(sma), XLoc::bar_index, false, false);
            lf = pf_linefill_new(_pf_linefills_, lv, lv2);
        } else {
            pf_line_set_y1(_pf_lines_, lv, (double)(sma));
            pf_line_set_y2(_pf_lines_, lv, (double)(sma));
            pf_noop(lv, pine_color::new_color(pine_color::blue, (int)(50)));
            pf_noop(lv, 2);
            pf_noop(lv, 0);
        }
        lbl = pf_label_new(_pf_labels_, (int64_t)(bar_index_), (double)(current_bar_.high), std::to_string(sma), XLoc::bar_index, YLoc::price);
        pf_label_set_text(_pf_labels_, lbl, std::string("x"));
        level = pf_line_get_y2(_pf_lines_, lv);
        eUp = (is_first_tick_ ? _ta_crossover_2.compute(current_bar_.close, level) : _ta_crossover_2.recompute(current_bar_.close, level));
        eDn = (is_first_tick_ ? _ta_crossunder_3.compute(current_bar_.close, level) : _ta_crossunder_3.recompute(current_bar_.close, level));
        if (eUp) {
            strategy_entry(std::string("L"), true, na<double>(), na<double>(), na<double>(), "");
        }
        if (((signed_position_size() > 0) && eDn)) {
