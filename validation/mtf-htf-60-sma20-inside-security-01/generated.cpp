...
public:
    ...
    explicit GeneratedStrategy() : _ta_sma_1(20), _sec0__ta_sma_1(20) {
    ...
    void on_bar(const Bar& bar) override {
        ...
        htfCl.push(_req_sec_1);
        longCond = ((htfCl[0] > htfSma[0]) && (htfCl[1] <= htfSma[1]));
        if (longCond) {
            strategy_entry(std::string("L"), true, na<double>(), na<double>(), 1, "", "", 0, -1);
        }
        exitCond = ((htfCl[0] < htfSma[0]) && (signed_position_size() > 0));
        if (exitCond) {
