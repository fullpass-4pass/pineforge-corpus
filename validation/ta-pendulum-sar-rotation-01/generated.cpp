...
public:
    ta::SAR _ta_sar_1;
    std::vector<double> _precalc__ta_sar_1;
    ta::EMA _ta_ema_2;
    ...
    double sarStart = 0.0;
    double sarIncrement = 0.0;
    double sarMaximum = 0.0;
    int trendLength = 0;
    ...
    struct _PFScriptState {
        decltype(GeneratedStrategy::_ta_sar_1) _pf_value_0;
        ...
        decltype(GeneratedStrategy::entryBar) _pf_value_4;
        decltype(GeneratedStrategy::_strat_position_size) _pf_value_5;
        decltype(GeneratedStrategy::sarStart) _pf_value_6;
        decltype(GeneratedStrategy::sarIncrement) _pf_value_7;
        decltype(GeneratedStrategy::sarMaximum) _pf_value_8;
        decltype(GeneratedStrategy::trendLength) _pf_value_9;
        decltype(GeneratedStrategy::maximumHoldBars) _pf_value_10;
        decltype(GeneratedStrategy::sarLine) _pf_value_11;
        decltype(GeneratedStrategy::trendLine) _pf_value_12;
        decltype(GeneratedStrategy::enterLong) _pf_value_13;
        decltype(GeneratedStrategy::ageExceeded) _pf_value_14;
        decltype(GeneratedStrategy::rotationEnded) _pf_value_15;
    ...
    void restore_script_state() override {
        ...
        this->_ta_sar_1 = _pf_script_state_checkpoint_->_pf_value_0;
        ...
        this->entryBar = _pf_script_state_checkpoint_->_pf_value_4;
        this->_strat_position_size = _pf_script_state_checkpoint_->_pf_value_5;
        this->sarStart = _pf_script_state_checkpoint_->_pf_value_6;
        this->sarIncrement = _pf_script_state_checkpoint_->_pf_value_7;
        this->sarMaximum = _pf_script_state_checkpoint_->_pf_value_8;
        this->trendLength = _pf_script_state_checkpoint_->_pf_value_9;
        this->maximumHoldBars = _pf_script_state_checkpoint_->_pf_value_10;
        this->sarLine = _pf_script_state_checkpoint_->_pf_value_11;
        this->trendLine = _pf_script_state_checkpoint_->_pf_value_12;
        this->enterLong = _pf_script_state_checkpoint_->_pf_value_13;
        this->ageExceeded = _pf_script_state_checkpoint_->_pf_value_14;
        this->rotationEnded = _pf_script_state_checkpoint_->_pf_value_15;
    ...
    explicit GeneratedStrategy() : _ta_sar_1(0.018, 0.021, 0.19), _ta_ema_2(73), entryBar(na<int>()) {
    ...
    void on_bar(const Bar& bar) override {
        ...
        if (!_inputs_initialized_) {
            ...
            sarMaximum = get_input_double("SAR Maximum", 0.19);
        ...
        sarLine = (history_advances_new_bar() ? _ta_sar_1.compute(current_bar_.high, current_bar_.low, current_bar_.close) : _ta_sar_1.recompute(current_bar_.high, current_bar_.low, current_bar_.close));
        trendLine = (history_advances_new_bar() ? _ta_ema_2.compute(current_bar_.close) : _ta_ema_2.recompute(current_bar_.close));
        enterLong = ((history_advances_new_bar() ? _ta_crossover_3.compute(current_bar_.close, sarLine) : _ta_crossover_3.recompute(current_bar_.close, sarLine)) && ([&]{ auto _pna_l = (current_bar_.close); auto _pna_r = (trendLine); double _pfc_l = static_cast<double>(_pna_l); double _pfc_r = static_cast<double>(_pna_r); bool _pfc_eq = (_pfc_l == _pfc_r) || (std::isfinite(_pfc_l) && std::isfinite(_pfc_r) && std::fabs(_pfc_l - _pfc_r) <= 1e-10); return !is_na(_pna_l) && !is_na(_pna_r) && ((_pfc_l > _pfc_r) && !_pfc_eq); }()));
        if ((([&]{ auto _pna_l = (signed_position_size()); auto _pna_r = (0); double _pfc_l = static_cast<double>(_pna_l); double _pfc_r = static_cast<double>(_pna_r); bool _pfc_eq = (_pfc_l == _pfc_r) || (std::isfinite(_pfc_l) && std::isfinite(_pfc_r) && std::fabs(_pfc_l - _pfc_r) <= 1e-10); return !is_na(_pna_l) && !is_na(_pna_r) && (_pfc_eq); }()) && enterLong)) {
            strategy_entry(std::string("Pendulum Long"), true, na<double>(), na<double>(), na<double>(), "");
            entryBar = pine_bar_index();
        }
        ageExceeded = ((([&]{ auto _pna_l = (signed_position_size()); auto _pna_r = (0); double _pfc_l = static_cast<double>(_pna_l); double _pfc_r = static_cast<double>(_pna_r); bool _pfc_eq = (_pfc_l == _pfc_r) || (std::isfinite(_pfc_l) && std::isfinite(_pfc_r) && std::fabs(_pfc_l - _pfc_r) <= 1e-10); return !is_na(_pna_l) && !is_na(_pna_r) && ((_pfc_l > _pfc_r) && !_pfc_eq); }()) && !(is_na(entryBar))) && ([&]{ auto _pna_l = ((pine_bar_index() - entryBar)); auto _pna_r = (maximumHoldBars); double _pfc_l = static_cast<double>(_pna_l); double _pfc_r = static_cast<double>(_pna_r); bool _pfc_eq = (_pfc_l == _pfc_r) || (std::isfinite(_pfc_l) && std::isfinite(_pfc_r) && std::fabs(_pfc_l - _pfc_r) <= 1e-10); return !is_na(_pna_l) && !is_na(_pna_r) && ((_pfc_l > _pfc_r) || _pfc_eq); }()));
        rotationEnded = (([&]{ auto _pna_l = (signed_position_size()); auto _pna_r = (0); double _pfc_l = static_cast<double>(_pna_l); double _pfc_r = static_cast<double>(_pna_r); bool _pfc_eq = (_pfc_l == _pfc_r) || (std::isfinite(_pfc_l) && std::isfinite(_pfc_r) && std::fabs(_pfc_l - _pfc_r) <= 1e-10); return !is_na(_pna_l) && !is_na(_pna_r) && ((_pfc_l > _pfc_r) && !_pfc_eq); }()) && ((history_advances_new_bar() ? _ta_crossunder_4.compute(current_bar_.close, sarLine) : _ta_crossunder_4.recompute(current_bar_.close, sarLine)) || ([&]{ auto _pna_l = (current_bar_.close); auto _pna_r = (trendLine); double _pfc_l = static_cast<double>(_pna_l); double _pfc_r = static_cast<double>(_pna_r); bool _pfc_eq = (_pfc_l == _pfc_r) || (std::isfinite(_pfc_l) && std::isfinite(_pfc_r) && std::fabs(_pfc_l - _pfc_r) <= 1e-10); return !is_na(_pna_l) && !is_na(_pna_r) && ((_pfc_l < _pfc_r) && !_pfc_eq); }())));
        if ((rotationEnded || ageExceeded)) {
            strategy_close(std::string("Pendulum Long"), ((ageExceeded) ? (std::string("Age guard")) : (std::string("SAR rotation"))), na<double>(), na<double>(), false, 137438953491ULL);
        }
