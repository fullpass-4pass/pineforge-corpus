...
public:
    ...
    explicit GeneratedStrategy() : _ta_rsi_1(14), _ta_ema_2(3) {
    ...
    void on_bar(const Bar& bar) override {
        ...
        if (!_inputs_initialized_) {
            i_rsi = get_input_int("RSI Length", 14);
        ...
        rsiValue = (is_first_tick_ ? _ta_rsi_1.compute(current_bar_.close) : _ta_rsi_1.recompute(current_bar_.close));
        rsiSmooth.push((is_first_tick_ ? _ta_ema_2.compute(rsiValue) : _ta_ema_2.recompute(rsiValue)));
        ...
        if ((rsiDivBull && (signed_position_size() == 0))) {
            strategy_entry(std::string("L"), true, na<double>(), na<double>(), 1, std::string("rsi div bull"), "", 0, -1);
        }
        if ((rsiDivBear && (signed_position_size() > 0))) {
            strategy_close(std::string("L"), std::string("rsi div bear exit"), na<double>(), na<double>(), false);
        }
    }
