...
public:
    ...
    explicit GeneratedStrategy() : _ta_bb_1(20, 2.0), _ta_rsi_2(14) {
    ...
    void on_bar(const Bar& bar) override {
        if (!_inputs_initialized_) {
            ...
            rsiOB = get_input_int("RSI Overbought", 70);
            rsiOS = get_input_int("RSI Oversold", 30);
        ...
        auto _result__ta_bb_1 = (is_first_tick_ ? _ta_bb_1.compute(current_bar_.close) : _ta_bb_1.recompute(current_bar_.close));
        double bbMid = _result__ta_bb_1.middle;
        double bbUpper = _result__ta_bb_1.upper;
        double bbLower = _result__ta_bb_1.lower;
        rsiVal = (is_first_tick_ ? _ta_rsi_2.compute(current_bar_.close) : _ta_rsi_2.recompute(current_bar_.close));
        longCond = ((current_bar_.close < bbLower) && (rsiVal < rsiOS));
        shortCond = ((current_bar_.close > bbUpper) && (rsiVal > rsiOB));
        if (longCond) {
        ...
        if (((signed_position_size() > 0) && (current_bar_.close > bbMid))) {
