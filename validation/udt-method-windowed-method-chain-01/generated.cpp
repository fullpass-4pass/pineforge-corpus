...
public:
    ...
    double _udt_Stat_zscore(Stat& self) {
        return (((is_na(self.sd) || (self.sd == 0.0))) ? (0.0) : (((double)((self.last - self.mean)) / (double)(self.sd))));
    }
    ...
    void on_bar(const Bar& bar) override {
        if (!_var_initialized) {
            stat = Stat{.last = na<double>(), .mean = na<double>(), .sd = na<double>(), .n = 0};
