...
public:
    ...
    void on_bar(const Bar& bar) override {
        ...
        levelSrc = (is_first_tick_ ? _ta_sma_1.compute(current_bar_.close) : _ta_sma_1.recompute(current_bar_.close));
        level = na<double>();
        if (!(is_na(levelSrc))) {
            if (is_na(res)) {
                res = pf_line_new(_pf_lines_, (int64_t)((bar_index_ - 1)), (double)(levelSrc), (int64_t)(bar_index_), (double)(levelSrc), XLoc::bar_index, false, true);
            } else {
                cur = pf_line_get_y2(_pf_lines_, res);
                pf_line_set_xy2(_pf_lines_, res, (int64_t)(bar_index_), (double)((cur + ((levelSrc - cur) * 0.35))));
            }
            level = pf_line_get_y2(_pf_lines_, res);
        }
        ready = !(is_na(level));
        eUp = (ready && (is_first_tick_ ? _ta_crossover_2.compute(current_bar_.close, level) : _ta_crossover_2.recompute(current_bar_.close, level)));
        eDn = (ready && (is_first_tick_ ? _ta_crossunder_3.compute(current_bar_.close, level) : _ta_crossunder_3.recompute(current_bar_.close, level)));
        if ((ready && eUp)) {
