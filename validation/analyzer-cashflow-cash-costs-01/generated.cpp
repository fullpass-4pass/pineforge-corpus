...
public:
    ...
    struct _PFScriptState {
        ...
        decltype(GeneratedStrategy::fastLine) _pf_value_9;
        decltype(GeneratedStrategy::slowLine) _pf_value_10;
        decltype(GeneratedStrategy::atrValue) _pf_value_11;
        decltype(GeneratedStrategy::enterLong) _pf_value_12;
        decltype(GeneratedStrategy::trendEnded) _pf_value_13;
    ...
    void restore_script_state() override {
        ...
        this->fastLine = _pf_script_state_checkpoint_->_pf_value_9;
        this->slowLine = _pf_script_state_checkpoint_->_pf_value_10;
        this->atrValue = _pf_script_state_checkpoint_->_pf_value_11;
        this->enterLong = _pf_script_state_checkpoint_->_pf_value_12;
        this->trendEnded = _pf_script_state_checkpoint_->_pf_value_13;
    ...
    explicit GeneratedStrategy() : _ta_ema_1(22), _ta_ema_2(63), _ta_atr_3(18) {
        initial_capital_ = 75000.0;
        default_qty_type_ = QtyType::CASH;
        default_qty_value_ = 4200.0;
        pyramiding_ = 0;
        commission_type_ = CommissionType::CASH_PER_CONTRACT;
        commission_value_ = 0.35;
        slippage_ = 1;
        margin_long_ = 100.0;
        margin_short_ = 100.0;
    ...
    void on_bar(const Bar& bar) override {
        if (!_inputs_initialized_) {
            fastLength = get_input_int("Fast EMA Length", 22);
            slowLength = get_input_int("Slow EMA Length", 63);
            atrLength = get_input_int("ATR Length", 18);
            maximumAtrLoss = get_input_double("Maximum ATR Loss", 2.6);
            _inputs_initialized_ = true;
        }
        if (!_ta_initialized_) {
            _ta_ema_1 = ta::EMA(get_input_int("Fast EMA Length", 22));
            _ta_ema_2 = ta::EMA(get_input_int("Slow EMA Length", 63));
        ...
        if ((([&]{ auto _pna_l = (signed_position_size()); auto _pna_r = (0); double _pfc_l = static_cast<double>(_pna_l); double _pfc_r = static_cast<double>(_pna_r); bool _pfc_eq = (_pfc_l == _pfc_r) || (std::isfinite(_pfc_l) && std::isfinite(_pfc_r) && std::fabs(_pfc_l - _pfc_r) <= 1e-10); return !is_na(_pna_l) && !is_na(_pna_r) && (_pfc_eq); }()) && enterLong)) {
            strategy_entry(std::string("Cashflow Long"), true, na<double>(), na<double>(), na<double>(), "");
        } else
        if ((([&]{ auto _pna_l = (signed_position_size()); auto _pna_r = (0); double _pfc_l = static_cast<double>(_pna_l); double _pfc_r = static_cast<double>(_pna_r); bool _pfc_eq = (_pfc_l == _pfc_r) || (std::isfinite(_pfc_l) && std::isfinite(_pfc_r) && std::fabs(_pfc_l - _pfc_r) <= 1e-10); return !is_na(_pna_l) && !is_na(_pna_r) && ((_pfc_l > _pfc_r) && !_pfc_eq); }()) && (trendEnded || riskExceeded))) {
            strategy_close(std::string("Cashflow Long"), ((riskExceeded) ? (std::string("Cash risk")) : (std::string("Trend ended"))), na<double>(), na<double>(), false, 124554051603ULL);
        }
    }
