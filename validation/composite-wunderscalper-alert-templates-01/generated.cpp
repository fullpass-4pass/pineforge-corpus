...
public:
    ...
    void on_bar(const Bar& bar) override {
        ...
        if (long_signal) {
            strategy_entry(std::string("L"), true, na<double>(), na<double>(), 1, std::string("alert long"), "", 0, -1);
        }
        if (short_signal) {
            strategy_entry(std::string("S"), false, na<double>(), na<double>(), 1, std::string("alert short"), "", 0, -1);
        }
    }
