...
public:
    ...
    void on_bar(const Bar& bar) override {
        if (!_var_initialized) {
            ...
            if (is_first_tick_) filterDir.push(filterDir[0]);
        }
        ...
        avgRange = (is_first_tick_ ? _ta_ema_1.compute((current_bar_.high - current_bar_.low)) : _ta_ema_1.recompute((current_bar_.high - current_bar_.low)));
        smoothRng = (avgRange * filterMult);
        hiTarget = (rangeFilter + smoothRng);
        loTarget = (rangeFilter - smoothRng);
        if ((current_bar_.close > hiTarget)) {
            rangeFilter = (current_bar_.close - smoothRng);
            filterDir.update(1);
        } else
        if ((current_bar_.close < loTarget)) {
            rangeFilter = (current_bar_.close + smoothRng);
            filterDir.update((-1));
        } else
        if ((filterDir[0] == 1)) {
            newFilter = (current_bar_.close - smoothRng);
            rangeFilter = std::max((double)(rangeFilter), (double)(newFilter));
        } else
        if ((filterDir[0] == (-1))) {
            newFilter = (current_bar_.close + smoothRng);
            rangeFilter = std::min((double)(rangeFilter), (double)(newFilter));
        }
        longCond = ((filterDir[0] == 1) && (filterDir[1] != 1));
        shortCond = ((filterDir[0] == (-1)) && (filterDir[1] != (-1)));
        if (longCond) {
