...
public:
    ...
    explicit GeneratedStrategy() : _ta_rsi_1(14), _sec0__ta_rsi_1(14) {
    ...
    void on_bar(const Bar& bar) override {
        htfRsi = _req_sec_0;
        longCond = (is_first_tick_ ? _ta_crossover_2.compute(htfRsi, 50) : _ta_crossover_2.recompute(htfRsi, 50));
        shortCond = (is_first_tick_ ? _ta_crossunder_3.compute(htfRsi, 50) : _ta_crossunder_3.recompute(htfRsi, 50));
        if (longCond) {
            strategy_entry(std::string("L"), true, na<double>(), na<double>(), 1, "", "", 0, -1);
        }
        if ((shortCond && (signed_position_size() > 0))) {
