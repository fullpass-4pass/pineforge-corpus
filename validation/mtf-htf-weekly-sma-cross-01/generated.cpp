...
public:
    ...
    ta::SMA _sec0__ta_sma_1;
    double wsma = 0.0;
    bool _ta_initialized_ = false;
    bool _inputs_initialized_ = false;

    explicit GeneratedStrategy() : _ta_sma_1(10), _sec0__ta_sma_1(10) {
    ...
    void configure_security_evaluators() override {
        ...
        register_security_eval(0, "W", input_tf_, false, false);
    }

    void on_bar(const Bar& bar) override {
        wsma = _req_sec_0;
        if ((is_first_tick_ ? _ta_crossover_2.compute(current_bar_.close, wsma) : _ta_crossover_2.recompute(current_bar_.close, wsma))) {
            strategy_entry(std::string("L"), true, na<double>(), na<double>(), na<double>(), std::string("close x up weekly SMA"));
        }
        if ((is_first_tick_ ? _ta_crossunder_3.compute(current_bar_.close, wsma) : _ta_crossunder_3.recompute(current_bar_.close, wsma))) {
            strategy_entry(std::string("S"), false, na<double>(), na<double>(), na<double>(), std::string("close x dn weekly SMA"));
        }
    }
    ...
    void _eval_security_0(const Bar& bar, bool is_complete) {
        auto _secval_0 = is_complete ? _sec0__ta_sma_1.compute(bar.close) : _sec0__ta_sma_1.recompute(bar.close);
