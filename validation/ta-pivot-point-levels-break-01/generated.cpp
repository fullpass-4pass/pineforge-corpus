...
public:
    ...
    void on_bar(const Bar& bar) override {
        ...
        R1 = pivots[1];
        S1 = pivots[2];
        if ((is_first_tick_ ? _ta_crossover_1.compute(current_bar_.close, R1) : _ta_crossover_1.recompute(current_bar_.close, R1))) {
            strategy_entry(std::string("L"), true, na<double>(), na<double>(), na<double>(), std::string("close x> R1"));
        }
        if ((is_first_tick_ ? _ta_crossunder_2.compute(current_bar_.close, S1) : _ta_crossunder_2.recompute(current_bar_.close, S1))) {
