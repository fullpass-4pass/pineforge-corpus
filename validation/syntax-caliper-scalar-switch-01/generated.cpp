...
public:
    ...
    std::string filterMode = std::string("");
    int baseLength = 0;
    double lengthScale = 0.0;
    int confirmationLength = 0;
    bool allowShort = false;
    int atrLength = 0;
    double stopAtr = 0.0;
    int scaledLength = 0;
    double emaFilter = 0.0;
    double smaFilter = 0.0;
    double selectedFilter = 0.0;
    double confirmationFilter = 0.0;
    double atrValue = 0.0;
    ...
    struct _PFScriptState {
        ...
        decltype(GeneratedStrategy::filterMode) _pf_value_6;
        decltype(GeneratedStrategy::baseLength) _pf_value_7;
        decltype(GeneratedStrategy::lengthScale) _pf_value_8;
        decltype(GeneratedStrategy::confirmationLength) _pf_value_9;
        decltype(GeneratedStrategy::allowShort) _pf_value_10;
        decltype(GeneratedStrategy::atrLength) _pf_value_11;
        decltype(GeneratedStrategy::stopAtr) _pf_value_12;
        decltype(GeneratedStrategy::scaledLength) _pf_value_13;
        decltype(GeneratedStrategy::emaFilter) _pf_value_14;
        decltype(GeneratedStrategy::smaFilter) _pf_value_15;
        decltype(GeneratedStrategy::selectedFilter) _pf_value_16;
        decltype(GeneratedStrategy::confirmationFilter) _pf_value_17;
    ...
    void restore_script_state() override {
        ...
        this->filterMode = _pf_script_state_checkpoint_->_pf_value_6;
        this->baseLength = _pf_script_state_checkpoint_->_pf_value_7;
        this->lengthScale = _pf_script_state_checkpoint_->_pf_value_8;
        this->confirmationLength = _pf_script_state_checkpoint_->_pf_value_9;
        this->allowShort = _pf_script_state_checkpoint_->_pf_value_10;
        this->atrLength = _pf_script_state_checkpoint_->_pf_value_11;
        this->stopAtr = _pf_script_state_checkpoint_->_pf_value_12;
        this->scaledLength = _pf_script_state_checkpoint_->_pf_value_13;
        this->emaFilter = _pf_script_state_checkpoint_->_pf_value_14;
        this->smaFilter = _pf_script_state_checkpoint_->_pf_value_15;
        this->selectedFilter = _pf_script_state_checkpoint_->_pf_value_16;
        this->confirmationFilter = _pf_script_state_checkpoint_->_pf_value_17;
    ...
    explicit GeneratedStrategy() : _ta_ema_1(1), _ta_sma_2(1), _ta_ema_3(113), _ta_atr_4(15) {
    ...
    void on_bar(const Bar& bar) override {
        if (!_inputs_initialized_) {
            filterMode = get_input_string("Filter Mode", std::string("EMA"));
            baseLength = get_input_int("Base Length", 29);
            lengthScale = get_input_double("Length Scale", 1.35);
            confirmationLength = get_input_int("Confirmation EMA", 113);
        ...
        if (__switch_val_0 == std::string("EMA")) {
            selectedFilter = emaFilter;
        }
        else if (__switch_val_0 == std::string("SMA")) {
            selectedFilter = smaFilter;
        }
        else {
            selectedFilter = ((double)((emaFilter + smaFilter)) / (double)(2.0));
        }
        confirmationFilter = (history_advances_new_bar() ? _ta_ema_3.compute(current_bar_.close) : _ta_ema_3.recompute(current_bar_.close));
        ...
        if ((history_advances_new_bar() ? _ta_crossover_5.compute(selectedFilter, confirmationFilter) : _ta_crossover_5.recompute(selectedFilter, confirmationFilter))) {
            strategy_entry(std::string("Caliper Long"), true, na<double>(), na<double>(), na<double>(), "");
        }
        if ((allowShort && (history_advances_new_bar() ? _ta_crossunder_6.compute(selectedFilter, confirmationFilter) : _ta_crossunder_6.recompute(selectedFilter, confirmationFilter)))) {
            strategy_entry(std::string("Caliper Short"), false, na<double>(), na<double>(), na<double>(), "");
        }
        if (([&]{ auto _pna_l = (signed_position_size()); auto _pna_r = (0); double _pfc_l = static_cast<double>(_pna_l); double _pfc_r = static_cast<double>(_pna_r); bool _pfc_eq = (_pfc_l == _pfc_r) || (std::isfinite(_pfc_l) && std::isfinite(_pfc_r) && std::fabs(_pfc_l - _pfc_r) <= 1e-10); return !is_na(_pna_l) && !is_na(_pna_r) && ((_pfc_l > _pfc_r) && !_pfc_eq); }())) {
            strategy_exit(std::string("Caliper Long Guard"), std::string("Caliper Long"), na<double>(), ((signed_position_size() == 0.0 ? na<double>() : position_entry_price_) - (atrValue * stopAtr)), na<double>(), na<double>(), na<double>(), 100.0, "", na<double>(), "", na<double>(), na<double>());
        }
        if (([&]{ auto _pna_l = (signed_position_size()); auto _pna_r = (0); double _pfc_l = static_cast<double>(_pna_l); double _pfc_r = static_cast<double>(_pna_r); bool _pfc_eq = (_pfc_l == _pfc_r) || (std::isfinite(_pfc_l) && std::isfinite(_pfc_r) && std::fabs(_pfc_l - _pfc_r) <= 1e-10); return !is_na(_pna_l) && !is_na(_pna_r) && ((_pfc_l < _pfc_r) && !_pfc_eq); }())) {
            strategy_exit(std::string("Caliper Short Guard"), std::string("Caliper Short"), na<double>(), ((signed_position_size() == 0.0 ? na<double>() : position_entry_price_) + (atrValue * stopAtr)), na<double>(), na<double>(), na<double>(), 100.0, "", na<double>(), "", na<double>(), na<double>());
        }
    }
