...
public:
    ta::HighestBars _ta_highestbars_1;
    std::vector<double> _precalc__ta_highestbars_1;
    ta::LowestBars _ta_lowestbars_2;
    std::vector<double> _precalc__ta_lowestbars_2;
    ...
    int lookback = 0;
    double barsToHigh = 0.0;
    double barsToLow = 0.0;
    double hh = 0.0;
    double ll = 0.0;
    bool newHigh = false;
    bool newLow = false;
    ...
    explicit GeneratedStrategy() : _ta_highestbars_1(20), _ta_lowestbars_2(20), _ta_highest_3(20), _ta_lowest_4(20), _ta_ema_5(50) {
    ...
    void on_bar(const Bar& bar) override {
        if (!_inputs_initialized_) {
            lookback = get_input_int("Lookback", 20);
        ...
        barsToHigh = (is_first_tick_ ? _ta_highestbars_1.compute(current_bar_.high) : _ta_highestbars_1.recompute(current_bar_.high));
        barsToLow = (is_first_tick_ ? _ta_lowestbars_2.compute(current_bar_.low) : _ta_lowestbars_2.recompute(current_bar_.low));
        hh = (is_first_tick_ ? _ta_highest_3.compute(current_bar_.high) : _ta_highest_3.recompute(current_bar_.high));
        ll = (is_first_tick_ ? _ta_lowest_4.compute(current_bar_.low) : _ta_lowest_4.recompute(current_bar_.low));
        newHigh = (barsToHigh == 0);
        newLow = (barsToLow == 0);
        maVal = (is_first_tick_ ? _ta_ema_5.compute(current_bar_.close) : _ta_ema_5.recompute(current_bar_.close));
        longCond = (newHigh && (current_bar_.close > maVal));
        shortCond = (newLow && (current_bar_.close < maVal));
        if (longCond) {
        ...
        midChan = ((double)((hh + ll)) / (double)(2));
        if (((signed_position_size() > 0) && (current_bar_.close < midChan))) {
    ...
    void precalculate(const Bar* bars, int n) {
        ...
        _precalc__ta_highestbars_1.resize(n);
        _precalc__ta_lowestbars_2.resize(n);
        ...
        for (int i = 0; i < n; ++i) {
            _precalc__ta_highestbars_1[i] = _ta_highestbars_1.compute(bars[i].high);
            _precalc__ta_lowestbars_2[i] = _ta_lowestbars_2.compute(bars[i].low);
