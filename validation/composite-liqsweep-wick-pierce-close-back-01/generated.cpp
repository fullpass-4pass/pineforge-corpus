...
public:
    ...
    void on_bar(const Bar& bar) override {
        ...
        if ((sweep_low && (signed_position_size() <= 0))) {
            strategy_entry(std::string("L"), true, na<double>(), na<double>(), 1, std::string("bullish sweep long"), "", 0, -1);
        }
        if ((sweep_high && (signed_position_size() >= 0))) {
            strategy_entry(std::string("S"), false, na<double>(), na<double>(), 1, std::string("bearish sweep short"), "", 0, -1);
        }
    }
