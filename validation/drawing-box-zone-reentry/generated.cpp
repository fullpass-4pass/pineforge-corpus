...
public:
    ...
    void on_bar(const Bar& bar) override {
        ...
        gapUp = (current_bar_.low > _s_high[2]);
        if (gapUp) {
            zones.push_back(pf_box_new(_pf_boxes_, (int64_t)((bar_index_ - 2)), (double)(current_bar_.low), (int64_t)(bar_index_), (double)(_s_high[2]), XLoc::bar_index));
        }
        n = (double)zones.size();
        if ((n > 0)) {
            z = zones[((n - 1))];
            ...
            if ((((signed_position_size() == 0) && (current_bar_.close <= top)) && (current_bar_.close >= bot))) {
                strategy_entry(std::string("L"), true, na<double>(), na<double>(), na<double>(), "");
            }
            if (((signed_position_size() > 0) && (current_bar_.close > top))) {
                strategy_close(std::string("L"), "", na<double>(), na<double>(), false);
                pf_box_delete(_pf_boxes_, zones[((n - 1))]);
                [&](){ auto v=zones.back(); zones.pop_back(); return v; }();
            }
        }
        if (((double)zones.size() > 20)) {
            pf_box_delete(_pf_boxes_, zones[(0)]);
            [&](){ auto v=zones.front(); zones.erase(zones.begin()); return v; }();
        }
    }
