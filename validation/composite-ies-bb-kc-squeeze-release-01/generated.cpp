...
public:
    ...
    explicit GeneratedStrategy() : _ta_sma_1(20), _ta_stdev_2(20), _ta_ema_3(20), _ta_atr_4(20) {
    ...
    void on_bar(const Bar& bar) override {
        ...
        bb_basis = (is_first_tick_ ? _ta_sma_1.compute(current_bar_.close) : _ta_sma_1.recompute(current_bar_.close));
        bb_dev = ((_use_precalc ? _precalc__ta_stdev_2[bar_index_] : (is_first_tick_ ? _ta_stdev_2.compute(current_bar_.close) : _ta_stdev_2.recompute(current_bar_.close))) * i_bb_mult);
        bb_upper = (bb_basis + bb_dev);
        bb_lower = (bb_basis - bb_dev);
        kc_basis = (is_first_tick_ ? _ta_ema_3.compute(current_bar_.close) : _ta_ema_3.recompute(current_bar_.close));
        kc_range = ((_use_precalc ? _precalc__ta_atr_4[bar_index_] : (is_first_tick_ ? _ta_atr_4.compute(current_bar_.high, current_bar_.low, current_bar_.close) : _ta_atr_4.recompute(current_bar_.high, current_bar_.low, current_bar_.close))) * i_kc_mult);
        kc_upper = (kc_basis + kc_range);
        kc_lower = (kc_basis - kc_range);
        squeeze_on.push(((bb_lower > kc_lower) && (bb_upper < kc_upper)));
        squeeze_release = (squeeze_on[1] && !(squeeze_on[0]));
        if ((squeeze_release && (signed_position_size() == 0))) {
            strategy_entry(std::string("L"), true, na<double>(), na<double>(), 1, std::string("squeeze release"), "", 0, -1);
        }
        if (((is_first_tick_ ? _ta_crossunder_5.compute(current_bar_.close, kc_basis) : _ta_crossunder_5.recompute(current_bar_.close, kc_basis)) && (signed_position_size() > 0))) {
            strategy_close(std::string("L"), std::string("kc basis lost"), na<double>(), na<double>(), false);
        }
    }
