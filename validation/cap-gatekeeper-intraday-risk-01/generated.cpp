...
public:
    ...
    std::string tradeSession = std::string("");
    std::string sessionTimezone = std::string("");
    int maxDailyFills = 0;
    int fastLength = 0;
    int slowLength = 0;
    int atrLength = 0;
    ...
    struct _PFScriptState {
        ...
        decltype(GeneratedStrategy::tradeSession) _pf_value_4;
        decltype(GeneratedStrategy::sessionTimezone) _pf_value_5;
        decltype(GeneratedStrategy::maxDailyFills) _pf_value_6;
        ...
        decltype(GeneratedStrategy::newDay) _pf_value_11;
        decltype(GeneratedStrategy::inSession) _pf_value_12;
        decltype(GeneratedStrategy::fastEma) _pf_value_13;
        decltype(GeneratedStrategy::slowEma) _pf_value_14;
    ...
    void restore_script_state() override {
        ...
        this->tradeSession = _pf_script_state_checkpoint_->_pf_value_4;
        this->sessionTimezone = _pf_script_state_checkpoint_->_pf_value_5;
        this->maxDailyFills = _pf_script_state_checkpoint_->_pf_value_6;
        ...
        this->newDay = _pf_script_state_checkpoint_->_pf_value_11;
        this->inSession = _pf_script_state_checkpoint_->_pf_value_12;
        this->fastEma = _pf_script_state_checkpoint_->_pf_value_13;
        this->slowEma = _pf_script_state_checkpoint_->_pf_value_14;
    ...
    explicit GeneratedStrategy() : _ta_ema_1(10), _ta_ema_2(31), _ta_atr_3(14) {
    ...
    void on_bar(const Bar& bar) override {
        if (!_inputs_initialized_) {
            tradeSession = get_input_string("Trade Session", std::string("0900-1700"));
            sessionTimezone = get_input_string("Session Timezone", std::string("Asia/Taipei"));
            maxDailyFills = get_input_int("Maximum Daily Filled Orders", 4);
            fastLength = get_input_int("Fast EMA", 10);
            slowLength = get_input_int("Slow EMA", 31);
            atrLength = get_input_int("ATR Length", 14);
            stopAtr = get_input_double("Stop ATR", 1.8);
            _inputs_initialized_ = true;
        }
        if (!_ta_initialized_) {
            _ta_ema_1 = ta::EMA(get_input_int("Fast EMA", 10));
            _ta_ema_2 = ta::EMA(get_input_int("Slow EMA", 31));
            _ta_atr_3 = ta::ATR(get_input_int("ATR Length", 14));
            _ta_initialized_ = true;
        }
        max_intraday_filled_orders_ = (int)(maxDailyFills);
        newDay = tf_change(prev_bar_timestamp_, current_bar_.timestamp, std::string("1D"));
        inSession = !(is_na(pine_time(current_bar_.timestamp, script_tf_, tradeSession, sessionTimezone, script_tf_)));
        ...
        if ((inSession && (history_advances_new_bar() ? _ta_crossover_4.compute(fastEma, slowEma) : _ta_crossover_4.recompute(fastEma, slowEma)))) {
            strategy_entry(std::string("Gatekeeper Long"), true, na<double>(), na<double>(), na<double>(), "");
        }
        if ((([&]{ auto _pna_l = (signed_position_size()); auto _pna_r = (0); double _pfc_l = static_cast<double>(_pna_l); double _pfc_r = static_cast<double>(_pna_r); bool _pfc_eq = (_pfc_l == _pfc_r) || (std::isfinite(_pfc_l) && std::isfinite(_pfc_r) && std::fabs(_pfc_l - _pfc_r) <= 1e-10); return !is_na(_pna_l) && !is_na(_pna_r) && ((_pfc_l > _pfc_r) && !_pfc_eq); }()) && (!(inSession) || newDay))) {
            strategy_close(std::string("Gatekeeper Long"), std::string("Session boundary"), na<double>(), na<double>(), false, 137438953491ULL);
        }
        if (([&]{ auto _pna_l = (signed_position_size()); auto _pna_r = (0); double _pfc_l = static_cast<double>(_pna_l); double _pfc_r = static_cast<double>(_pna_r); bool _pfc_eq = (_pfc_l == _pfc_r) || (std::isfinite(_pfc_l) && std::isfinite(_pfc_r) && std::fabs(_pfc_l - _pfc_r) <= 1e-10); return !is_na(_pna_l) && !is_na(_pna_r) && ((_pfc_l > _pfc_r) && !_pfc_eq); }())) {
            strategy_exit(std::string("Gatekeeper Guard"), std::string("Gatekeeper Long"), na<double>(), ((signed_position_size() == 0.0 ? na<double>() : position_entry_price_) - (atrValue * stopAtr)), na<double>(), na<double>(), na<double>(), 100.0, "", na<double>(), "", na<double>(), na<double>());
        }
    }
