...
public:
    ...
    int rsiLength = 0;
    double upperTurn = 0.0;
    double lowerTurn = 0.0;
    int atrLength = 0;
    double stopAtr = 0.0;
    double rsiValue = 0.0;
    double atrValue = 0.0;
    ...
    struct _PFScriptState {
        ...
        decltype(GeneratedStrategy::rsiLength) _pf_value_4;
        decltype(GeneratedStrategy::upperTurn) _pf_value_5;
        decltype(GeneratedStrategy::lowerTurn) _pf_value_6;
        decltype(GeneratedStrategy::atrLength) _pf_value_7;
        decltype(GeneratedStrategy::stopAtr) _pf_value_8;
        decltype(GeneratedStrategy::rsiValue) _pf_value_9;
        decltype(GeneratedStrategy::atrValue) _pf_value_10;
        decltype(GeneratedStrategy::bullTurn) _pf_value_11;
        decltype(GeneratedStrategy::bearTurn) _pf_value_12;
    ...
    void restore_script_state() override {
        ...
        this->rsiLength = _pf_script_state_checkpoint_->_pf_value_4;
        this->upperTurn = _pf_script_state_checkpoint_->_pf_value_5;
        this->lowerTurn = _pf_script_state_checkpoint_->_pf_value_6;
        this->atrLength = _pf_script_state_checkpoint_->_pf_value_7;
        this->stopAtr = _pf_script_state_checkpoint_->_pf_value_8;
        this->rsiValue = _pf_script_state_checkpoint_->_pf_value_9;
        this->atrValue = _pf_script_state_checkpoint_->_pf_value_10;
        this->bullTurn = _pf_script_state_checkpoint_->_pf_value_11;
        this->bearTurn = _pf_script_state_checkpoint_->_pf_value_12;
    ...
    explicit GeneratedStrategy() : _ta_rsi_1(17), _ta_atr_2(14) {
        initial_capital_ = 100000.0;
        default_qty_type_ = QtyType::PERCENT_OF_EQUITY;
        default_qty_value_ = 8.0;
        pyramiding_ = 0;
    ...
    void on_bar(const Bar& bar) override {
        if (!_inputs_initialized_) {
            rsiLength = get_input_int("RSI Length", 17);
            upperTurn = get_input_double("Bull Threshold", 58.0);
            lowerTurn = get_input_double("Bear Threshold", 42.0);
        ...
        bullTurn = (history_advances_new_bar() ? _ta_crossover_3.compute(rsiValue, upperTurn) : _ta_crossover_3.recompute(rsiValue, upperTurn));
        bearTurn = (history_advances_new_bar() ? _ta_crossunder_4.compute(rsiValue, lowerTurn) : _ta_crossunder_4.recompute(rsiValue, lowerTurn));
        if (bullTurn) {
            if (([&]{ auto _pna_l = (signed_position_size()); auto _pna_r = (0); double _pfc_l = static_cast<double>(_pna_l); double _pfc_r = static_cast<double>(_pna_r); bool _pfc_eq = (_pfc_l == _pfc_r) || (std::isfinite(_pfc_l) && std::isfinite(_pfc_r) && std::fabs(_pfc_l - _pfc_r) <= 1e-10); return !is_na(_pna_l) && !is_na(_pna_r) && ((_pfc_l < _pfc_r) && !_pfc_eq); }())) {
                strategy_close(std::string("Crosscurrent Short"), std::string("Close before opposite market entry"), na<double>(), na<double>(), false, 120259084311ULL);
            }
            strategy_entry(std::string("Crosscurrent Long"), true, na<double>(), na<double>(), na<double>(), "");
        }
        if (bearTurn) {
            if (([&]{ auto _pna_l = (signed_position_size()); auto _pna_r = (0); double _pfc_l = static_cast<double>(_pna_l); double _pfc_r = static_cast<double>(_pna_r); bool _pfc_eq = (_pfc_l == _pfc_r) || (std::isfinite(_pfc_l) && std::isfinite(_pfc_r) && std::fabs(_pfc_l - _pfc_r) <= 1e-10); return !is_na(_pna_l) && !is_na(_pna_r) && ((_pfc_l > _pfc_r) && !_pfc_eq); }())) {
                strategy_close(std::string("Crosscurrent Long"), std::string("Close before opposite market entry"), na<double>(), na<double>(), false, 141733920791ULL);
            }
            strategy_entry(std::string("Crosscurrent Short"), false, na<double>(), na<double>(), na<double>(), "");
        }
        if (([&]{ auto _pna_l = (signed_position_size()); auto _pna_r = (0); double _pfc_l = static_cast<double>(_pna_l); double _pfc_r = static_cast<double>(_pna_r); bool _pfc_eq = (_pfc_l == _pfc_r) || (std::isfinite(_pfc_l) && std::isfinite(_pfc_r) && std::fabs(_pfc_l - _pfc_r) <= 1e-10); return !is_na(_pna_l) && !is_na(_pna_r) && ((_pfc_l > _pfc_r) && !_pfc_eq); }())) {
            strategy_exit(std::string("Crosscurrent Long Guard"), std::string("Crosscurrent Long"), na<double>(), ((signed_position_size() == 0.0 ? na<double>() : position_entry_price_) - (atrValue * stopAtr)), na<double>(), na<double>(), na<double>(), 100.0, "", na<double>(), "", na<double>(), na<double>());
        }
        if (([&]{ auto _pna_l = (signed_position_size()); auto _pna_r = (0); double _pfc_l = static_cast<double>(_pna_l); double _pfc_r = static_cast<double>(_pna_r); bool _pfc_eq = (_pfc_l == _pfc_r) || (std::isfinite(_pfc_l) && std::isfinite(_pfc_r) && std::fabs(_pfc_l - _pfc_r) <= 1e-10); return !is_na(_pna_l) && !is_na(_pna_r) && ((_pfc_l < _pfc_r) && !_pfc_eq); }())) {
            strategy_exit(std::string("Crosscurrent Short Guard"), std::string("Crosscurrent Short"), na<double>(), ((signed_position_size() == 0.0 ? na<double>() : position_entry_price_) + (atrValue * stopAtr)), na<double>(), na<double>(), na<double>(), 100.0, "", na<double>(), "", na<double>(), na<double>());
        }
    }
