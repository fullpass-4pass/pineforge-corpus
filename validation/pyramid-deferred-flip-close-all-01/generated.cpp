...
public:
    ...
    struct _PFScriptState {
        ...
        decltype(GeneratedStrategy::rsiVal) _pf_value_6;
        decltype(GeneratedStrategy::emaFast) _pf_value_7;
        decltype(GeneratedStrategy::emaSlow) _pf_value_8;
        decltype(GeneratedStrategy::atrVal) _pf_value_9;
        decltype(GeneratedStrategy::longCond) _pf_value_10;
        decltype(GeneratedStrategy::shortStop) _pf_value_11;
    ...
    void restore_script_state() override {
        ...
        this->rsiVal = _pf_script_state_checkpoint_->_pf_value_6;
        this->emaFast = _pf_script_state_checkpoint_->_pf_value_7;
        this->emaSlow = _pf_script_state_checkpoint_->_pf_value_8;
        this->atrVal = _pf_script_state_checkpoint_->_pf_value_9;
        this->longCond = _pf_script_state_checkpoint_->_pf_value_10;
        this->shortStop = _pf_script_state_checkpoint_->_pf_value_11;
    ...
    explicit GeneratedStrategy() : _ta_rsi_1(14), _ta_ema_2(9), _ta_ema_3(21), _ta_atr_4(14) {
    ...
    void on_bar(const Bar& bar) override {
        ...
        atrVal = (history_advances_new_bar() ? _ta_atr_4.compute(current_bar_.high, current_bar_.low, current_bar_.close) : _ta_atr_4.recompute(current_bar_.high, current_bar_.low, current_bar_.close));
        longCond = ((history_advances_new_bar() ? _ta_crossover_5.compute(emaFast, emaSlow) : _ta_crossover_5.recompute(emaFast, emaSlow)) && ([&]{ auto _pna_l = (rsiVal); auto _pna_r = (70); double _pfc_l = static_cast<double>(_pna_l); double _pfc_r = static_cast<double>(_pna_r); bool _pfc_eq = (_pfc_l == _pfc_r) || (std::isfinite(_pfc_l) && std::isfinite(_pfc_r) && std::fabs(_pfc_l - _pfc_r) <= 1e-10); return !is_na(_pna_l) && !is_na(_pna_r) && ((_pfc_l < _pfc_r) && !_pfc_eq); }()));
        shortStop = (history_advances_new_bar() ? _ta_crossunder_6.compute(rsiVal, 50) : _ta_crossunder_6.recompute(rsiVal, 50));
        if (longCond) {
            strategy_entry(std::string("L"), true, na<double>(), na<double>(), 1, std::string("add long market"), "", 0, -1);
        }
        if (shortStop) {
            strategy_entry(std::string("S"), false, na<double>(), (current_bar_.low - (atrVal * 0.25)), 1, std::string("flip short stop"), "", 0, -1);
        }
        if ((([&]{ auto _pna_l = (pine_hour(current_bar_.timestamp, syminfo_.timezone)); auto _pna_r = (21); double _pfc_l = static_cast<double>(_pna_l); double _pfc_r = static_cast<double>(_pna_r); bool _pfc_eq = (_pfc_l == _pfc_r) || (std::isfinite(_pfc_l) && std::isfinite(_pfc_r) && std::fabs(_pfc_l - _pfc_r) <= 1e-10); return !is_na(_pna_l) && !is_na(_pna_r) && (_pfc_eq); }()) && ([&]{ auto _pna_l = (pine_minute(current_bar_.timestamp, syminfo_.timezone)); auto _pna_r = (45); double _pfc_l = static_cast<double>(_pna_l); double _pfc_r = static_cast<double>(_pna_r); bool _pfc_eq = (_pfc_l == _pfc_r) || (std::isfinite(_pfc_l) && std::isfinite(_pfc_r) && std::fabs(_pfc_l - _pfc_r) <= 1e-10); return !is_na(_pna_l) && !is_na(_pna_r) && (_pfc_eq); }()))) {
            strategy_close("", std::string("session close_all"), na<double>(), na<double>(), false);
        }
    }
