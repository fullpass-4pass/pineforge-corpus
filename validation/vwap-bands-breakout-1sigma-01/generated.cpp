...
public:
    ...
    explicit GeneratedStrategy() : _ta_vwap_bands_1(1) {
    ...
    void on_bar(const Bar& bar) override {
        auto _result__ta_vwap_bands_1 = (is_first_tick_ ? _ta_vwap_bands_1.compute(current_bar_.close, current_bar_.volume, current_bar_.timestamp) : _ta_vwap_bands_1.recompute(current_bar_.close, current_bar_.volume, current_bar_.timestamp));
        ...
        long_condition = (is_first_tick_ ? _ta_crossover_2.compute(current_bar_.close, upper_band) : _ta_crossover_2.recompute(current_bar_.close, upper_band));
        exit_condition = (is_first_tick_ ? _ta_crossunder_3.compute(current_bar_.close, vw) : _ta_crossunder_3.recompute(current_bar_.close, vw));
        if (long_condition) {
    ...
    void precalculate(const Bar* bars, int n) {
        ...
        _ta_vwap_bands_1 = ta::VWAPBands(1);


        for (int i = 0; i < n; ++i) {
            _precalc__ta_vwap_bands_1[i] = _ta_vwap_bands_1.compute(bars[i].close, bars[i].volume, bars[i].timestamp);
        }
