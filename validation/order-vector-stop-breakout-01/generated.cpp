...
public:
    ...
    struct _PFScriptState {
        ...
        decltype(GeneratedStrategy::entryBar) _pf_value_3;
        decltype(GeneratedStrategy::_strat_position_size) _pf_value_4;
        decltype(GeneratedStrategy::_hist_call_1) _pf_value_5;
        decltype(GeneratedStrategy::breakoutLength) _pf_value_6;
        decltype(GeneratedStrategy::atrLength) _pf_value_7;
        decltype(GeneratedStrategy::contractionRatio) _pf_value_8;
        decltype(GeneratedStrategy::entryBufferAtr) _pf_value_9;
        decltype(GeneratedStrategy::stopAtr) _pf_value_10;
        decltype(GeneratedStrategy::maxBars) _pf_value_11;
        decltype(GeneratedStrategy::atrValue) _pf_value_12;
        decltype(GeneratedStrategy::atrBaseline) _pf_value_13;
        decltype(GeneratedStrategy::breakoutLevel) _pf_value_14;
        decltype(GeneratedStrategy::contracted) _pf_value_15;
        decltype(GeneratedStrategy::stopTrigger) _pf_value_16;
        decltype(GeneratedStrategy::_var_initialized) _pf_value_17;
    ...
    void restore_script_state() override {
        ...
        this->entryBar = _pf_script_state_checkpoint_->_pf_value_3;
        this->_strat_position_size = _pf_script_state_checkpoint_->_pf_value_4;
        this->_hist_call_1 = _pf_script_state_checkpoint_->_pf_value_5;
        this->breakoutLength = _pf_script_state_checkpoint_->_pf_value_6;
        this->atrLength = _pf_script_state_checkpoint_->_pf_value_7;
        this->contractionRatio = _pf_script_state_checkpoint_->_pf_value_8;
        this->entryBufferAtr = _pf_script_state_checkpoint_->_pf_value_9;
        this->stopAtr = _pf_script_state_checkpoint_->_pf_value_10;
        this->maxBars = _pf_script_state_checkpoint_->_pf_value_11;
        this->atrValue = _pf_script_state_checkpoint_->_pf_value_12;
        this->atrBaseline = _pf_script_state_checkpoint_->_pf_value_13;
        this->breakoutLevel = _pf_script_state_checkpoint_->_pf_value_14;
        this->contracted = _pf_script_state_checkpoint_->_pf_value_15;
        this->stopTrigger = _pf_script_state_checkpoint_->_pf_value_16;
        this->_var_initialized = _pf_script_state_checkpoint_->_pf_value_17;
    ...
    explicit GeneratedStrategy() : _ta_atr_1(15), _ta_sma_2(28), _ta_highest_3(28), entryBar(na<int>()) {
    ...
    void on_bar(const Bar& bar) override {
        ...
        if (!_inputs_initialized_) {
            ...
            contractionRatio = get_input_double("ATR Contraction Ratio", 0.82);
            entryBufferAtr = get_input_double("Entry Buffer ATR", 0.12);
        ...
        if (!_ta_initialized_) {
            ...
            _ta_sma_2 = ta::SMA(get_input_int("Breakout Length", 28));
        ...
        atrBaseline = (history_advances_new_bar() ? _ta_sma_2.compute(atrValue) : _ta_sma_2.recompute(atrValue));
        breakoutLevel = ([&]() -> double { if (_use_precalc) { auto _pf_hist_offset_raw = (1); if (is_na(_pf_hist_offset_raw)) return na<double>(); long double _pf_hist_offset_numeric = static_cast<long double>(_pf_hist_offset_raw); if (bar_index_ < 0 || _pf_hist_offset_numeric < 0.0L || _pf_hist_offset_numeric > static_cast<long double>(bar_index_)) return na<double>(); std::size_t _pf_hist_offset = static_cast<std::size_t>(_pf_hist_offset_numeric); std::size_t _pf_hist_bar = static_cast<std::size_t>(bar_index_) - _pf_hist_offset; if (_pf_hist_bar >= _precalc__ta_highest_3.size()) return na<double>(); return _precalc__ta_highest_3[(std::size_t)_pf_hist_bar]; } double _hv = ((_use_precalc ? _precalc__ta_highest_3[bar_index_] : (history_advances_new_bar() ? _ta_highest_3.compute(current_bar_.high) : _ta_highest_3.recompute(current_bar_.high)))); if (history_advances_new_bar()) _hist_call_1.push(_hv); else _hist_call_1.update(_hv); return _hist_call_1[(int)(1)]; }());
        contracted = ([&]{ auto _pna_l = (atrValue); auto _pna_r = ((atrBaseline * contractionRatio)); double _pfc_l = static_cast<double>(_pna_l); double _pfc_r = static_cast<double>(_pna_r); bool _pfc_eq = (_pfc_l == _pfc_r) || (std::isfinite(_pfc_l) && std::isfinite(_pfc_r) && std::fabs(_pfc_l - _pfc_r) <= 1e-10); return !is_na(_pna_l) && !is_na(_pna_r) && ((_pfc_l < _pfc_r) && !_pfc_eq); }());
        stopTrigger = (breakoutLevel + (atrValue * entryBufferAtr));
        if ((([&]{ auto _pna_l = (signed_position_size()); auto _pna_r = (0); double _pfc_l = static_cast<double>(_pna_l); double _pfc_r = static_cast<double>(_pna_r); bool _pfc_eq = (_pfc_l == _pfc_r) || (std::isfinite(_pfc_l) && std::isfinite(_pfc_r) && std::fabs(_pfc_l - _pfc_r) <= 1e-10); return !is_na(_pna_l) && !is_na(_pna_r) && (_pfc_eq); }()) && contracted)) {
            strategy_entry(std::string("Vector Breakout"), true, na<double>(), stopTrigger, na<double>(), "", "", 0, -1);
        } else
        if (([&]{ auto _pna_l = (signed_position_size()); auto _pna_r = (0); double _pfc_l = static_cast<double>(_pna_l); double _pfc_r = static_cast<double>(_pna_r); bool _pfc_eq = (_pfc_l == _pfc_r) || (std::isfinite(_pfc_l) && std::isfinite(_pfc_r) && std::fabs(_pfc_l - _pfc_r) <= 1e-10); return !is_na(_pna_l) && !is_na(_pna_r) && (_pfc_eq); }())) {
            strategy_cancel(std::string("Vector Breakout"));
        }
        ...
        if (([&]{ auto _pna_l = (signed_position_size()); auto _pna_r = (0); double _pfc_l = static_cast<double>(_pna_l); double _pfc_r = static_cast<double>(_pna_r); bool _pfc_eq = (_pfc_l == _pfc_r) || (std::isfinite(_pfc_l) && std::isfinite(_pfc_r) && std::fabs(_pfc_l - _pfc_r) <= 1e-10); return !is_na(_pna_l) && !is_na(_pna_r) && ((_pfc_l > _pfc_r) && !_pfc_eq); }())) {
            strategy_exit(std::string("Vector Guard"), std::string("Vector Breakout"), na<double>(), ((signed_position_size() == 0.0 ? na<double>() : position_entry_price_) - (atrValue * stopAtr)), na<double>(), na<double>(), na<double>(), 100.0, "", na<double>(), "", na<double>(), na<double>());
            if ((!(is_na(entryBar)) && ([&]{ auto _pna_l = ((pine_bar_index() - entryBar)); auto _pna_r = (maxBars); double _pfc_l = static_cast<double>(_pna_l); double _pfc_r = static_cast<double>(_pna_r); bool _pfc_eq = (_pfc_l == _pfc_r) || (std::isfinite(_pfc_l) && std::isfinite(_pfc_r) && std::fabs(_pfc_l - _pfc_r) <= 1e-10); return !is_na(_pna_l) && !is_na(_pna_r) && ((_pfc_l > _pfc_r) || _pfc_eq); }()))) {
                strategy_close(std::string("Vector Breakout"), std::string("Time boundary"), na<double>(), na<double>(), false, 180388626455ULL);
            }
        }
    }
