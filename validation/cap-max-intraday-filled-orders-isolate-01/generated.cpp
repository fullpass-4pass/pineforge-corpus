...
public:
    ...
    explicit GeneratedStrategy() : _ta_sma_1(5), _ta_sma_2(13) {
    ...
    void on_bar(const Bar& bar) override {
        if (!_inputs_initialized_) {
            i_fast = get_input_int("Fast MA", 5);
            i_slow = get_input_int("Slow MA", 13);
            i_max_fills = get_input_int("Max intraday filled orders", 5);
            _inputs_initialized_ = true;
        }
        if (!_ta_initialized_) {
            _ta_sma_1 = ta::SMA(get_input_int("Fast MA", 5));
            _ta_sma_2 = ta::SMA(get_input_int("Slow MA", 13));
