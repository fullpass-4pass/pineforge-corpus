...
public:
    ...
    explicit GeneratedStrategy() : _ta_percentrank_1(50), _ta_sma_2(5) {
    ...
    void on_bar(const Bar& bar) override {
        ...
        if (!_inputs_initialized_) {
            prLen = get_input_int("Percentile Rank Length", 50);
            obPR = get_input_double("Overbought PR", 90);
            osPR = get_input_double("Oversold PR", 10);
            _inputs_initialized_ = true;
        }
        if (!_ta_initialized_) {
            _ta_percentrank_1 = ta::PercentRank(get_input_int("Percentile Rank Length", 50));
            _ta_initialized_ = true;
        }
        priceChange = (current_bar_.close - _s_close[1]);
        pr.push((is_first_tick_ ? _ta_percentrank_1.compute(priceChange) : _ta_percentrank_1.recompute(priceChange)));
        prSmooth = (is_first_tick_ ? _ta_sma_2.compute(pr[0]) : _ta_sma_2.recompute(pr[0]));
        longCond = ((pr[0] < osPR) && (pr[0] > pr[1]));
        shortCond = ((pr[0] > obPR) && (pr[0] < pr[1]));
        if (longCond) {
