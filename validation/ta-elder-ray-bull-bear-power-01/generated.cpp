...
public:
    ...
    Series<double> bearPower;
    Series<double> bullPower;
    int emaLen = 0;
    double emaVal = 0.0;
    double emaTrend = 0.0;
    bool upTrend = false;
    ...
    explicit GeneratedStrategy() : _ta_ema_1(13), _ta_ema_2(50) {
    ...
    void on_bar(const Bar& bar) override {
        if (!_inputs_initialized_) {
            emaLen = get_input_int("EMA Length", 13);
        ...
        emaVal = (is_first_tick_ ? _ta_ema_1.compute(current_bar_.close) : _ta_ema_1.recompute(current_bar_.close));
        bullPower.push((current_bar_.high - emaVal));
        bearPower.push((current_bar_.low - emaVal));
        emaTrend = (is_first_tick_ ? _ta_ema_2.compute(current_bar_.close) : _ta_ema_2.recompute(current_bar_.close));
        upTrend = (current_bar_.close > emaTrend);
        dnTrend = (current_bar_.close < emaTrend);
        longCond = ((upTrend && (bearPower[0] < 0)) && (bearPower[0] > bearPower[1]));
        shortCond = ((dnTrend && (bullPower[0] > 0)) && (bullPower[0] < bullPower[1]));
        if (longCond) {
        ...
        if ((((signed_position_size() > 0) && (bearPower[0] > 0)) && (bearPower[0] < bearPower[1]))) {
            strategy_close(std::string("Long"), "", na<double>(), na<double>(), false);
        }
        if ((((signed_position_size() < 0) && (bullPower[0] < 0)) && (bullPower[0] > bullPower[1]))) {
