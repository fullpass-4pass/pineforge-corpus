...
public:
    ...
    explicit GeneratedStrategy() : _ta_median_1(20), _ta_ema_2(20) {
    ...
    void on_bar(const Bar& bar) override {
        if (!_inputs_initialized_) {
            medLen = get_input_int("Median Length", 20);
            emaLen = get_input_int("EMA Length", 20);
        ...
        medVal = (is_first_tick_ ? _ta_median_1.compute(current_bar_.close) : _ta_median_1.recompute(current_bar_.close));
        emaVal = (is_first_tick_ ? _ta_ema_2.compute(current_bar_.close) : _ta_ema_2.recompute(current_bar_.close));
        longCond = (is_first_tick_ ? _ta_crossover_3.compute(medVal, emaVal) : _ta_crossover_3.recompute(medVal, emaVal));
        shortCond = (is_first_tick_ ? _ta_crossunder_4.compute(medVal, emaVal) : _ta_crossunder_4.recompute(medVal, emaVal));
        if (longCond) {
