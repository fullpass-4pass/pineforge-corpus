...
struct Trend {
    int lookback = 3;
    static Trend create() { return Trend{}; }
};
...
public:
    ...
    double _udt_Trend_changeRate(Trend& self) {
        double ref = _s_close[self.lookback];
        return (((ref == 0.0)) ? (0.0) : (((double)((current_bar_.close - ref)) / (double)(ref))));
    }

    double _udt_Trend_swingRange(Trend& self) {
        double hi = _s_high[self.lookback];
        double lo = _s_low[self.lookback];
        return (hi - lo);
    }
    ...
    void on_bar(const Bar& bar) override {
        ...
        if (!_var_initialized) {
            trend = Trend{.lookback = 3};
            _var_initialized = true;
        } else {
        }
        r = _udt_Trend_changeRate(trend);
        swing = _udt_Trend_swingRange(trend);
        ...
        if (((((is_first_tick_ ? _ta_crossover_3.compute(emaFast, emaSlow) : _ta_crossover_3.recompute(emaFast, emaSlow)) && (r > 0.01)) && (swing > 0)) && (signed_position_size() == 0))) {
