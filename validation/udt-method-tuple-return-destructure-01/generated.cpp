...
struct Bracket {
    double stop_mult = 1.5;
    double limit_mult = 3.0;
    double base_qty = 1.0;
    static Bracket create() { return Bracket{}; }
};
...
public:
    ...
    explicit GeneratedStrategy() : _ta_atr_1(14), _ta_ema_2(9), _ta_ema_3(21) {
    ...
    std::tuple<double, double, double> _udt_Bracket_levels(Bracket& self, double price, double atr, bool isLong) {
        double stopPx = ((isLong) ? ((price - (self.stop_mult * atr))) : ((price + (self.stop_mult * atr))));
        double limitPx = ((isLong) ? ((price + (self.limit_mult * atr))) : ((price - (self.limit_mult * atr))));
        return std::make_tuple(stopPx, limitPx, self.base_qty);
    }

    void on_bar(const Bar& bar) override {
        if (!_var_initialized) {
            br = Bracket{.stop_mult = 1.5, .limit_mult = 3.0, .base_qty = 1.0};
