...
public:
    ...
    explicit GeneratedStrategy() : _ta_ema_1(8), _ta_ema_2(21), tick_counter(0) {
    ...
    void on_bar(const Bar& bar) override {
        ...
        if (((cross_up && gate) && (signed_position_size() <= 0))) {
            strategy_entry(std::string("L"), true, na<double>(), na<double>(), 1, std::string("tick gated long"), "", 0, -1);
        }
        if (((cross_down && gate) && (signed_position_size() >= 0))) {
            strategy_entry(std::string("S"), false, na<double>(), na<double>(), 1, std::string("tick gated short"), "", 0, -1);
        }
    }
