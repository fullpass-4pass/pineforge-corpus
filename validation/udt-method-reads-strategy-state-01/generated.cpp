...
struct Risk {
    ...
    static Risk create() { return Risk{}; }
};
...
public:
    ...
    bool _udt_Risk_allowsEntry(Risk& self) {
        bool flatOk = (signed_position_size() == 0);
        bool cooldownOk = ((bar_index_ - self.last_exit_bar) >= self.cooldown_bars);
        double ddLimit = (initial_capital_ * self.max_drawdown_pct);
        bool ddOk = (net_profit() >= (-ddLimit));
        bool equityOk = ((current_equity() + open_profit(current_bar_.close)) > 0.0);
        return (((flatOk && cooldownOk) && ddOk) && equityOk);
    }

    void on_bar(const Bar& bar) override {
        if (!_var_initialized) {
            risk = Risk{.cooldown_bars = 5, .last_exit_bar = (-1000), .max_drawdown_pct = 0.05};
        ...
        if (((is_first_tick_ ? _ta_crossover_3.compute(emaFast, emaSlow) : _ta_crossover_3.recompute(emaFast, emaSlow)) && _udt_Risk_allowsEntry(risk))) {
