...
public:
    ...
    void on_bar(const Bar& bar) override {
        ...
        qty_dyn = ((double)(std::round((((double)((current_equity() + open_profit(current_bar_.close))) / (double)(current_bar_.close)) * 100))) / (double)(1000));
        if ((fire && (qty_dyn > 0))) {
            strategy_entry(std::string("E"), true, na<double>(), na<double>(), qty_dyn, std::string("qty = 10% equity / close"), "", 0, -1);
        }
