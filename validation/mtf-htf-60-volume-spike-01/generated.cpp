...
public:
    ...
    ta::SMA _sec1__ta_sma_1;
    double htfVol = 0.0;
    double htfVolSma = 0.0;
    bool spike = false;
    bool _ta_initialized_ = false;
    bool _inputs_initialized_ = false;

    explicit GeneratedStrategy() : _ta_sma_1(20), _sec1__ta_sma_1(20) {
    ...
    void on_bar(const Bar& bar) override {
        htfVol = _req_sec_0;
        htfVolSma = _req_sec_1;
        spike = (htfVol > (htfVolSma * 2.0));
        if ((spike && (signed_position_size() == 0))) {
            strategy_entry(std::string("V"), true, na<double>(), na<double>(), 1, "", "", 0, -1);
        }
        if ((!(spike) && (signed_position_size() > 0))) {
            strategy_close(std::string("V"), "", na<double>(), na<double>(), false);
        }
    }
    ...
    void _eval_security_0(const Bar& bar, bool is_complete) {
        _req_sec_0 = bar.volume;
    }

    void _eval_security_1(const Bar& bar, bool is_complete) {
        auto _secval_0 = is_complete ? _sec1__ta_sma_1.compute(bar.volume) : _sec1__ta_sma_1.recompute(bar.volume);
        _req_sec_1 = _secval_0;
    }
