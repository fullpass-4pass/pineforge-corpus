...
public:
    ...
    struct _PFScriptState {
        ...
        decltype(GeneratedStrategy::trancheTaken) _pf_value_5;
        decltype(GeneratedStrategy::fastLength) _pf_value_6;
        decltype(GeneratedStrategy::slowLength) _pf_value_7;
        decltype(GeneratedStrategy::atrLength) _pf_value_8;
        decltype(GeneratedStrategy::trancheAtr) _pf_value_9;
        decltype(GeneratedStrategy::maximumAtrLoss) _pf_value_10;
        decltype(GeneratedStrategy::fastLine) _pf_value_11;
        decltype(GeneratedStrategy::slowLine) _pf_value_12;
        decltype(GeneratedStrategy::atrValue) _pf_value_13;
        decltype(GeneratedStrategy::enterLong) _pf_value_14;
        decltype(GeneratedStrategy::firstObjective) _pf_value_15;
        decltype(GeneratedStrategy::trendEnded) _pf_value_16;
    ...
    void restore_script_state() override {
        ...
        this->trancheTaken = _pf_script_state_checkpoint_->_pf_value_5;
        this->fastLength = _pf_script_state_checkpoint_->_pf_value_6;
        this->slowLength = _pf_script_state_checkpoint_->_pf_value_7;
        this->atrLength = _pf_script_state_checkpoint_->_pf_value_8;
        this->trancheAtr = _pf_script_state_checkpoint_->_pf_value_9;
        this->maximumAtrLoss = _pf_script_state_checkpoint_->_pf_value_10;
        this->fastLine = _pf_script_state_checkpoint_->_pf_value_11;
        this->slowLine = _pf_script_state_checkpoint_->_pf_value_12;
        this->atrValue = _pf_script_state_checkpoint_->_pf_value_13;
        this->enterLong = _pf_script_state_checkpoint_->_pf_value_14;
        this->firstObjective = _pf_script_state_checkpoint_->_pf_value_15;
        this->trendEnded = _pf_script_state_checkpoint_->_pf_value_16;
    ...
    explicit GeneratedStrategy() : _ta_ema_1(19), _ta_ema_2(58), _ta_atr_3(16), trancheTaken(false) {
    ...
    void on_bar(const Bar& bar) override {
        ...
        if (!_ta_initialized_) {
            ...
            _ta_ema_2 = ta::EMA(get_input_int("Slow EMA Length", 58));
        ...
        if ((([&]{ auto _pna_l = (signed_position_size()); auto _pna_r = (0); double _pfc_l = static_cast<double>(_pna_l); double _pfc_r = static_cast<double>(_pna_r); bool _pfc_eq = (_pfc_l == _pfc_r) || (std::isfinite(_pfc_l) && std::isfinite(_pfc_r) && std::fabs(_pfc_l - _pfc_r) <= 1e-10); return !is_na(_pna_l) && !is_na(_pna_r) && (_pfc_eq); }()) && enterLong)) {
            strategy_entry(std::string("Tranche Long"), true, na<double>(), na<double>(), na<double>(), "");
        } else
        if (((([&]{ auto _pna_l = (signed_position_size()); auto _pna_r = (0); double _pfc_l = static_cast<double>(_pna_l); double _pfc_r = static_cast<double>(_pna_r); bool _pfc_eq = (_pfc_l == _pfc_r) || (std::isfinite(_pfc_l) && std::isfinite(_pfc_r) && std::fabs(_pfc_l - _pfc_r) <= 1e-10); return !is_na(_pna_l) && !is_na(_pna_r) && ((_pfc_l > _pfc_r) && !_pfc_eq); }()) && !(trancheTaken)) && firstObjective)) {
            strategy_close(std::string("Tranche Long"), std::string("First tranche"), na<double>(), 50, false, 154618822675ULL);
            trancheTaken = true;
        } else
        if ((([&]{ auto _pna_l = (signed_position_size()); auto _pna_r = (0); double _pfc_l = static_cast<double>(_pna_l); double _pfc_r = static_cast<double>(_pna_r); bool _pfc_eq = (_pfc_l == _pfc_r) || (std::isfinite(_pfc_l) && std::isfinite(_pfc_r) && std::fabs(_pfc_l - _pfc_r) <= 1e-10); return !is_na(_pna_l) && !is_na(_pna_r) && ((_pfc_l > _pfc_r) && !_pfc_eq); }()) && (trendEnded || riskExceeded))) {
            strategy_close(std::string("Tranche Long"), ((riskExceeded) ? (std::string("ATR risk")) : (std::string("Trend ended"))), na<double>(), na<double>(), false, 167503724563ULL);
        }
    }
