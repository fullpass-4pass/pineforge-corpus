...
public:
    ...
    explicit GeneratedStrategy() : _ta_rsi_1(14), _ta_ema_2(9) {
    ...
    void on_bar(const Bar& bar) override {
        if (!_inputs_initialized_) {
            ...
            signalLen = get_input_int("Signal EMA Length", 9);
            _inputs_initialized_ = true;
        }
        if (!_ta_initialized_) {
            _ta_rsi_1 = ta::RSI(get_input_int("RSI Length", 14));
            _ta_ema_2 = ta::EMA(get_input_int("Signal EMA Length", 9));
            _ta_initialized_ = true;
        }
        rsiVal = (is_first_tick_ ? _ta_rsi_1.compute(current_bar_.close) : _ta_rsi_1.recompute(current_bar_.close));
        rsiSignal = (is_first_tick_ ? _ta_ema_2.compute(rsiVal) : _ta_ema_2.recompute(rsiVal));
        longCond = ((is_first_tick_ ? _ta_crossover_3.compute(rsiVal, rsiSignal) : _ta_crossover_3.recompute(rsiVal, rsiSignal)) && (rsiVal < 50));
        shortCond = ((is_first_tick_ ? _ta_crossunder_4.compute(rsiVal, rsiSignal) : _ta_crossunder_4.recompute(rsiVal, rsiSignal)) && (rsiVal > 50));
        if (longCond) {
