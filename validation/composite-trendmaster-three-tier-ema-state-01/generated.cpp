...
public:
    ...
    explicit GeneratedStrategy() : _ta_ema_1(21), _ta_ema_2(55), _ta_ema_3(200), stack_state(0), prev_state(0) {
    ...
    void on_bar(const Bar& bar) override {
        ...
        ema_mid = (is_first_tick_ ? _ta_ema_2.compute(current_bar_.close) : _ta_ema_2.recompute(current_bar_.close));
        ema_slow = (is_first_tick_ ? _ta_ema_3.compute(current_bar_.close) : _ta_ema_3.recompute(current_bar_.close));
        ...
        long_edge = ((stack_state == 1) && (prev_state != 1));
        short_edge = ((stack_state == (-1)) && (prev_state != (-1)));
        if ((long_edge && (signed_position_size() <= 0))) {
            strategy_entry(std::string("L"), true, na<double>(), na<double>(), 1, std::string("stack bull edge"), "", 0, -1);
        }
        if ((short_edge && (signed_position_size() >= 0))) {
            strategy_entry(std::string("S"), false, na<double>(), na<double>(), 1, std::string("stack bear edge"), "", 0, -1);
        }
