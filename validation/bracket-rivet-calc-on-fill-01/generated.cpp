...
public:
    ...
    struct _PFScriptState {
        ...
        decltype(GeneratedStrategy::targetAtr) _pf_value_8;
        decltype(GeneratedStrategy::fastEma) _pf_value_9;
        decltype(GeneratedStrategy::slowEma) _pf_value_10;
    ...
    void restore_script_state() override {
        ...
        this->targetAtr = _pf_script_state_checkpoint_->_pf_value_8;
        this->fastEma = _pf_script_state_checkpoint_->_pf_value_9;
        this->slowEma = _pf_script_state_checkpoint_->_pf_value_10;
    ...
    explicit GeneratedStrategy() : _ta_ema_1(11), _ta_ema_2(37), _ta_atr_3(15) {
    ...
    void on_bar(const Bar& bar) override {
        if (!_inputs_initialized_) {
            fastLength = get_input_int("Fast EMA", 11);
            slowLength = get_input_int("Slow EMA", 37);
            atrLength = get_input_int("ATR Length", 15);
            stopAtr = get_input_double("Stop ATR", 1.7);
            targetAtr = get_input_double("Target ATR", 2.2);
            _inputs_initialized_ = true;
        }
        if (!_ta_initialized_) {
            _ta_ema_1 = ta::EMA(get_input_int("Fast EMA", 11));
            _ta_ema_2 = ta::EMA(get_input_int("Slow EMA", 37));
        ...
        if (((history_advances_new_bar() ? _ta_crossover_4.compute(fastEma, slowEma) : _ta_crossover_4.recompute(fastEma, slowEma)) && ([&]{ auto _pna_l = (signed_position_size()); auto _pna_r = (0); double _pfc_l = static_cast<double>(_pna_l); double _pfc_r = static_cast<double>(_pna_r); bool _pfc_eq = (_pfc_l == _pfc_r) || (std::isfinite(_pfc_l) && std::isfinite(_pfc_r) && std::fabs(_pfc_l - _pfc_r) <= 1e-10); return !is_na(_pna_l) && !is_na(_pna_r) && (_pfc_eq); }()))) {
            strategy_entry(std::string("Rivet Long"), true, na<double>(), na<double>(), na<double>(), "");
        }
        if (([&]{ auto _pna_l = (signed_position_size()); auto _pna_r = (0); double _pfc_l = static_cast<double>(_pna_l); double _pfc_r = static_cast<double>(_pna_r); bool _pfc_eq = (_pfc_l == _pfc_r) || (std::isfinite(_pfc_l) && std::isfinite(_pfc_r) && std::fabs(_pfc_l - _pfc_r) <= 1e-10); return !is_na(_pna_l) && !is_na(_pna_r) && ((_pfc_l > _pfc_r) && !_pfc_eq); }())) {
            strategy_exit(std::string("Rivet Attached Risk"), std::string("Rivet Long"), ((signed_position_size() == 0.0 ? na<double>() : position_entry_price_) + (atrValue * targetAtr)), ((signed_position_size() == 0.0 ? na<double>() : position_entry_price_) - (atrValue * stopAtr)), na<double>(), na<double>(), na<double>(), 100.0, "", na<double>(), "", na<double>(), na<double>());
        }
    }
