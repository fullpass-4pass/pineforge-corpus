...
public:
    ...
    explicit GeneratedStrategy() : _ta_sma_1(20), _ta_stdev_2(20) {
    ...
    void on_bar(const Bar& bar) override {
        if (!_inputs_initialized_) {
            i_len = get_input_int("Baseline / stdev length", 20);
            i_mult = get_input_double("Expansion multiple", 2.0);
        ...
        baseline = (is_first_tick_ ? _ta_sma_1.compute(current_bar_.close) : _ta_sma_1.recompute(current_bar_.close));
        vol = (is_first_tick_ ? _ta_stdev_2.compute(current_bar_.close) : _ta_stdev_2.recompute(current_bar_.close));
        upper = (baseline + (vol * i_mult));
        lower = (baseline - (vol * i_mult));
        bull_break = (current_bar_.close > upper);
        bear_break = (current_bar_.close < lower);
        if ((bull_break && (signed_position_size() <= 0))) {
            if ((signed_position_size() < 0)) {
                strategy_close(std::string("S"), std::string("flip flat"), na<double>(), na<double>(), false);
            }
            strategy_entry(std::string("L"), true, na<double>(), na<double>(), na<double>(), std::string("upper-band break"));
        }
        if ((bear_break && (signed_position_size() >= 0))) {
            if ((signed_position_size() > 0)) {
                strategy_close(std::string("L"), std::string("flip flat"), na<double>(), na<double>(), false);
            }
            strategy_entry(std::string("S"), false, na<double>(), na<double>(), na<double>(), std::string("lower-band break"));
        }
    }
