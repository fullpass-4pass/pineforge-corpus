...
public:
    ...
    explicit GeneratedStrategy() : _ta_bb_1(20, 2.0), _ta_kc_2(20, 1.5), _ta_linreg_3(20), wasSqueezed(false) {
    ...
    void on_bar(const Bar& bar) override {
        ...
        double bbLower = _result__ta_bb_1.lower;
        auto _result__ta_kc_2 = (is_first_tick_ ? _ta_kc_2.compute(src, current_bar_.high, current_bar_.low, current_bar_.close) : _ta_kc_2.recompute(src, current_bar_.high, current_bar_.low, current_bar_.close));
        double kcMid = _result__ta_kc_2.middle;
        double kcUpper = _result__ta_kc_2.upper;
        double kcLower = _result__ta_kc_2.lower;
        sqzOn = ((bbLower > kcLower) && (bbUpper < kcUpper));
        sqzOff = ((bbLower < kcLower) || (bbUpper > kcUpper));
        mom = (is_first_tick_ ? _ta_linreg_3.compute((src - bbMid), 0) : _ta_linreg_3.recompute((src - bbMid), 0));
        if (sqzOn) {
            wasSqueezed = true;
        }
        sqzFired = (wasSqueezed && sqzOff);
        if ((sqzFired && (mom > 0))) {
        ...
        if (((signed_position_size() > 0) && (mom < 0))) {
    ...
    void precalculate(const Bar* bars, int n) {
        ...
        for (int i = 0; i < n; ++i) {
            _precalc__ta_bb_1[i] = _ta_bb_1.compute(src);
