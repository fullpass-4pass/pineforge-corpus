...
public:
    ...
    Series<double> htfEma;
    bool emaRising = false;
    bool emaFalling = false;
    bool _ta_initialized_ = false;
    bool _inputs_initialized_ = false;

    explicit GeneratedStrategy() : _ta_ema_1(26), _sec0__ta_ema_1(26) {
    ...
    void on_bar(const Bar& bar) override {
        htfEma.push(_req_sec_0);
        emaRising = ((!(is_na(htfEma[0])) && !(is_na(htfEma[1]))) && (htfEma[0] > htfEma[1]));
        emaFalling = ((!(is_na(htfEma[0])) && !(is_na(htfEma[1]))) && (htfEma[0] < htfEma[1]));
        if ((emaRising && (signed_position_size() <= 0))) {
            if ((signed_position_size() < 0)) {
                strategy_close(std::string("S"), std::string("flip to long"), na<double>(), na<double>(), false);
            }
            strategy_entry(std::string("L"), true, na<double>(), na<double>(), na<double>(), std::string("daily EMA rising"));
        }
        if ((emaFalling && (signed_position_size() >= 0))) {
            if ((signed_position_size() > 0)) {
                strategy_close(std::string("L"), std::string("flip to short"), na<double>(), na<double>(), false);
            }
            strategy_entry(std::string("S"), false, na<double>(), na<double>(), na<double>(), std::string("daily EMA falling"));
        }
    }
    ...
    void _eval_security_0(const Bar& bar, bool is_complete) {
        auto _secval_0 = is_complete ? _sec0__ta_ema_1.compute(bar.close) : _sec0__ta_ema_1.recompute(bar.close);
