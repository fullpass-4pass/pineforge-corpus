...
public:
    ...
    explicit GeneratedStrategy() : _ta_sma_1(5), _ta_sma_2(20) {
    ...
    void on_bar(const Bar& bar) override {
        if (!_inputs_initialized_) {
            i_fast = get_input_int("Fast SMA length", 5);
            i_slow = get_input_int("Slow SMA length", 20);
            _inputs_initialized_ = true;
        }
        if (!_ta_initialized_) {
            _ta_sma_1 = ta::SMA(get_input_int("Fast SMA length", 5));
            _ta_sma_2 = ta::SMA(get_input_int("Slow SMA length", 20));
        ...
        cross_up = (is_first_tick_ ? _ta_crossover_3.compute(fast, slow) : _ta_crossover_3.recompute(fast, slow));
        cross_down = (is_first_tick_ ? _ta_crossunder_4.compute(fast, slow) : _ta_crossunder_4.recompute(fast, slow));
        if ((cross_up && (signed_position_size() <= 0))) {
            strategy_entry(std::string("L"), true, na<double>(), na<double>(), 1, std::string("ma cross up"), "", 0, -1);
        }
        if ((cross_down && (signed_position_size() >= 0))) {
            strategy_entry(std::string("S"), false, na<double>(), na<double>(), 1, std::string("ma cross down"), "", 0, -1);
        }
    }
