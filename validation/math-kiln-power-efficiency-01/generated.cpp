...
public:
    ...
    struct _PFScriptState {
        ...
        decltype(GeneratedStrategy::efficiencyLength) _pf_value_6;
        decltype(GeneratedStrategy::minimumEfficiency) _pf_value_7;
        ...
        decltype(GeneratedStrategy::displacement) _pf_value_12;
        decltype(GeneratedStrategy::pathProxy) _pf_value_13;
        decltype(GeneratedStrategy::efficiency) _pf_value_14;
    ...
    void restore_script_state() override {
        ...
        this->efficiencyLength = _pf_script_state_checkpoint_->_pf_value_6;
        this->minimumEfficiency = _pf_script_state_checkpoint_->_pf_value_7;
        ...
        this->displacement = _pf_script_state_checkpoint_->_pf_value_12;
        this->pathProxy = _pf_script_state_checkpoint_->_pf_value_13;
        this->efficiency = _pf_script_state_checkpoint_->_pf_value_14;
    ...
    explicit GeneratedStrategy() : _ta_ema_1(43), _ta_atr_2(18) {
    ...
    void on_bar(const Bar& bar) override {
        ...
        if (!_inputs_initialized_) {
            ...
            efficiencyLength = get_input_int("Efficiency Window", 18);
            minimumEfficiency = get_input_double("Minimum Squared Efficiency", 0.035);
            atrLength = get_input_int("ATR Length", 18);
            maximumAtrLoss = get_input_double("Maximum ATR Loss", 2.7);
        ...
        trendEnded = ((history_advances_new_bar() ? _ta_crossunder_4.compute(current_bar_.close, trendLine) : _ta_crossunder_4.recompute(current_bar_.close, trendLine)) || ([&]{ auto _pna_l = (efficiency); auto _pna_r = ((minimumEfficiency * 0.35)); double _pfc_l = static_cast<double>(_pna_l); double _pfc_r = static_cast<double>(_pna_r); bool _pfc_eq = (_pfc_l == _pfc_r) || (std::isfinite(_pfc_l) && std::isfinite(_pfc_r) && std::fabs(_pfc_l - _pfc_r) <= 1e-10); return !is_na(_pna_l) && !is_na(_pna_r) && ((_pfc_l < _pfc_r) && !_pfc_eq); }()));
        ...
        if ((([&]{ auto _pna_l = (signed_position_size()); auto _pna_r = (0); double _pfc_l = static_cast<double>(_pna_l); double _pfc_r = static_cast<double>(_pna_r); bool _pfc_eq = (_pfc_l == _pfc_r) || (std::isfinite(_pfc_l) && std::isfinite(_pfc_r) && std::fabs(_pfc_l - _pfc_r) <= 1e-10); return !is_na(_pna_l) && !is_na(_pna_r) && (_pfc_eq); }()) && enterLong)) {
            strategy_entry(std::string("Kiln Long"), true, na<double>(), na<double>(), na<double>(), "");
        } else
        if ((([&]{ auto _pna_l = (signed_position_size()); auto _pna_r = (0); double _pfc_l = static_cast<double>(_pna_l); double _pfc_r = static_cast<double>(_pna_r); bool _pfc_eq = (_pfc_l == _pfc_r) || (std::isfinite(_pfc_l) && std::isfinite(_pfc_r) && std::fabs(_pfc_l - _pfc_r) <= 1e-10); return !is_na(_pna_l) && !is_na(_pna_r) && ((_pfc_l > _pfc_r) && !_pfc_eq); }()) && (trendEnded || riskExceeded))) {
            strategy_close(std::string("Kiln Long"), ((riskExceeded) ? (std::string("ATR risk")) : (std::string("Efficiency ended"))), na<double>(), na<double>(), false, 141733920787ULL);
        }
    }
