...
public:
    ta::Mom _ta_mom_1;
    std::vector<double> _precalc__ta_mom_1;
    ta::ROC _ta_roc_2;
    ...
    explicit GeneratedStrategy() : _ta_mom_1(10), _ta_roc_2(10) {
    ...
    void on_bar(const Bar& bar) override {
        if (!_inputs_initialized_) {
            momLen = get_input_int("Momentum Length", 10);
            rocLen = get_input_int("ROC Length", 10);
        ...
        momVal = (is_first_tick_ ? _ta_mom_1.compute(current_bar_.close) : _ta_mom_1.recompute(current_bar_.close));
        rocVal = (is_first_tick_ ? _ta_roc_2.compute(current_bar_.close) : _ta_roc_2.recompute(current_bar_.close));
        longCond = ((is_first_tick_ ? _ta_crossover_3.compute(momVal, 0) : _ta_crossover_3.recompute(momVal, 0)) && (rocVal > 0));
        shortCond = ((is_first_tick_ ? _ta_crossunder_4.compute(momVal, 0) : _ta_crossunder_4.recompute(momVal, 0)) && (rocVal < 0));
        if (longCond) {
        ...
        if (((signed_position_size() > 0) && (momVal < 0))) {
    ...
    void precalculate(const Bar* bars, int n) {
        ...
        for (int i = 0; i < n; ++i) {
            _precalc__ta_mom_1[i] = _ta_mom_1.compute(bars[i].close);
