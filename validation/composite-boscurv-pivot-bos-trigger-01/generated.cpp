...
public:
    ...
    void on_bar(const Bar& bar) override {
        ...
        if ((bos_long && (signed_position_size() <= 0))) {
            strategy_entry(std::string("L"), true, na<double>(), na<double>(), 1, std::string("bos long"), "", 0, -1);
        }
        if ((bos_short && (signed_position_size() >= 0))) {
            strategy_entry(std::string("S"), false, na<double>(), na<double>(), 1, std::string("bos short"), "", 0, -1);
        }
    }
