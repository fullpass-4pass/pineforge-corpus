...
public:
    ...
    void on_bar(const Bar& bar) override {
        ...
        r = (is_first_tick_ ? _ta_rsi_3.compute(current_bar_.close) : _ta_rsi_3.recompute(current_bar_.close));
        mom_bull = (r > i_rsi_lo);
        mom_bear = (r < i_rsi_hi);
        hi_now.push((is_first_tick_ ? _ta_highest_4.compute(current_bar_.high) : _ta_highest_4.recompute(current_bar_.high)));
        lo_now.push((is_first_tick_ ? _ta_lowest_5.compute(current_bar_.low) : _ta_lowest_5.recompute(current_bar_.low)));
        break_hi = hi_now[1];
        break_lo = lo_now[1];
