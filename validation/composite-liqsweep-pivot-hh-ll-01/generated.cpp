...
public:
    ...
    explicit GeneratedStrategy() : _ta_pivothigh_1(5, 5), _ta_pivotlow_2(5, 5), last_ph(na<double>()), prev_ph(na<double>()), last_pl(na<double>()), prev_pl(na<double>()) {
    ...
    void on_bar(const Bar& bar) override {
        ...
        higher_high = ((!(is_na(ph)) && !(is_na(prev_ph))) && (ph > prev_ph));
        lower_low = ((!(is_na(pl)) && !(is_na(prev_pl))) && (pl < prev_pl));
        if ((higher_high && (signed_position_size() <= 0))) {
            strategy_entry(std::string("L"), true, na<double>(), na<double>(), 1, std::string("hh long"), "", 0, -1);
        }
        if ((lower_low && (signed_position_size() >= 0))) {
            strategy_entry(std::string("S"), false, na<double>(), na<double>(), 1, std::string("ll short"), "", 0, -1);
        }
    }
