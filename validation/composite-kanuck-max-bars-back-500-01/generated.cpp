...
public:
    ...
    explicit GeneratedStrategy() : _ta_sma_1(400), _ta_rsi_2(14) {
    ...
    void on_bar(const Bar& bar) override {
        ...
        if (!_inputs_initialized_) {
            ...
            i_deep_lag = get_input_int("Close-reference lookback", 450);
        ...
        long_sma = (is_first_tick_ ? _ta_sma_1.compute(current_bar_.close) : _ta_sma_1.recompute(current_bar_.close));
        deep_rsi = (is_first_tick_ ? _ta_rsi_2.compute(_s_close[i_rsi_offset]) : _ta_rsi_2.recompute(_s_close[i_rsi_offset]));
        deep_close = _s_close[i_deep_lag];
        deep_ready = ((!(is_na(long_sma)) && !(is_na(deep_rsi))) && !(is_na(deep_close)));
        long_trigger = (deep_ready && (current_bar_.close > deep_close));
        short_trigger = (deep_ready && (current_bar_.close < deep_close));
        if ((long_trigger && (signed_position_size() <= 0))) {
            strategy_entry(std::string("L"), true, na<double>(), na<double>(), 1, std::string("deep-history long"), "", 0, -1);
        }
        if ((short_trigger && (signed_position_size() >= 0))) {
            strategy_entry(std::string("S"), false, na<double>(), na<double>(), 1, std::string("deep-history short"), "", 0, -1);
        }
    }
    ...
    void precalculate(const Bar* bars, int n) {
        ...
        for (int i = 0; i < n; ++i) {
            ...
            _precalc__ta_rsi_2[i] = _ta_rsi_2.compute(_s_close[i_rsi_offset]);
        }
