...
public:
    ...
    Series<double> base;
    double proj = 0.0;
    ChartPoint p1 = ChartPoint{};
    ChartPoint p2 = ChartPoint{};
    double ref = 0.0;
    bool ready = false;
    bool sDn = false;
    bool sUp = false;
    ...
    void on_bar(const Bar& bar) override {
        ...
        base.push((is_first_tick_ ? _ta_sma_1.compute(current_bar_.close) : _ta_sma_1.recompute(current_bar_.close)));
        proj = na<double>();
        if ((!(is_na(base[0])) && !(is_na(base[2])))) {
            p1 = ChartPoint{ .index=(int64_t)((bar_index_ - 2)), .time=na<int64_t>(), .price=(base[2]) };
            p2 = ChartPoint{ .index=(int64_t)(bar_index_), .time=na<int64_t>(), .price=(base[0]) };
            if (is_na(trend)) {
            ...
            proj = pf_line_get_price(_pf_lines_, trend, (int64_t)((bar_index_ - 1)));
        }
        ref = (is_na(proj) ? current_bar_.close : proj);
        ready = !(is_na(proj));
        sDn = (is_first_tick_ ? _ta_crossunder_2.compute(current_bar_.close, ref) : _ta_crossunder_2.recompute(current_bar_.close, ref));
        sUp = (is_first_tick_ ? _ta_crossover_3.compute(current_bar_.close, ref) : _ta_crossover_3.recompute(current_bar_.close, ref));
        if ((ready && sDn)) {
            strategy_entry(std::string("S"), false, na<double>(), na<double>(), na<double>(), "");
        }
        if ((((signed_position_size() < 0) && ready) && sUp)) {
            strategy_close(std::string("S"), "", na<double>(), na<double>(), false);
        }
    }
