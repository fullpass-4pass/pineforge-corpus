...
public:
    ...
    Series<bool> insideSession;
    std::string tradeSession = std::string("");
    std::string tradeTimezone = std::string("");
    int fastLength = 0;
    int slowLength = 0;
    int atrLength = 0;
    double stopAtr = 0.0;
    ...
    struct _PFScriptState {
        ...
        decltype(GeneratedStrategy::insideSession) _pf_value_4;
        decltype(GeneratedStrategy::tradeSession) _pf_value_5;
        decltype(GeneratedStrategy::tradeTimezone) _pf_value_6;
        ...
        decltype(GeneratedStrategy::wasInside) _pf_value_11;
        decltype(GeneratedStrategy::sessionOpened) _pf_value_12;
        decltype(GeneratedStrategy::sessionClosed) _pf_value_13;
        decltype(GeneratedStrategy::fastEma) _pf_value_14;
        decltype(GeneratedStrategy::slowEma) _pf_value_15;
    ...
    void restore_script_state() override {
        ...
        this->insideSession = _pf_script_state_checkpoint_->_pf_value_4;
        this->tradeSession = _pf_script_state_checkpoint_->_pf_value_5;
        this->tradeTimezone = _pf_script_state_checkpoint_->_pf_value_6;
        ...
        this->wasInside = _pf_script_state_checkpoint_->_pf_value_11;
        this->sessionOpened = _pf_script_state_checkpoint_->_pf_value_12;
        this->sessionClosed = _pf_script_state_checkpoint_->_pf_value_13;
        this->fastEma = _pf_script_state_checkpoint_->_pf_value_14;
        this->slowEma = _pf_script_state_checkpoint_->_pf_value_15;
    ...
    explicit GeneratedStrategy() : _ta_ema_1(16), _ta_ema_2(44), _ta_atr_3(14) {
    ...
    void on_bar(const Bar& bar) override {
        if (!_inputs_initialized_) {
            tradeSession = get_input_string("Trade Session", std::string("0800-1600"));
            tradeTimezone = get_input_string("Session Timezone", std::string("Etc/UTC"));
        ...
        if (history_advances_new_bar()) insideSession.push(!(is_na(pine_time(current_bar_.timestamp, script_tf_, tradeSession, tradeTimezone, script_tf_))));
        else insideSession.update(!(is_na(pine_time(current_bar_.timestamp, script_tf_, tradeSession, tradeTimezone, script_tf_))));
        wasInside = insideSession[1];
        sessionOpened = (insideSession[0] && !(wasInside));
        sessionClosed = (!(insideSession[0]) && wasInside);
        ...
        if ((insideSession[0] && (sessionOpened || (history_advances_new_bar() ? _ta_crossover_4.compute(fastEma, slowEma) : _ta_crossover_4.recompute(fastEma, slowEma))))) {
            strategy_entry(std::string("Compass Long"), true, na<double>(), na<double>(), na<double>(), "");
        }
        if (sessionClosed) {
            strategy_close(std::string("Compass Long"), std::string("Session flatten"), na<double>(), na<double>(), false, 141733920787ULL);
        }
        if (([&]{ auto _pna_l = (signed_position_size()); auto _pna_r = (0); double _pfc_l = static_cast<double>(_pna_l); double _pfc_r = static_cast<double>(_pna_r); bool _pfc_eq = (_pfc_l == _pfc_r) || (std::isfinite(_pfc_l) && std::isfinite(_pfc_r) && std::fabs(_pfc_l - _pfc_r) <= 1e-10); return !is_na(_pna_l) && !is_na(_pna_r) && ((_pfc_l > _pfc_r) && !_pfc_eq); }())) {
            strategy_exit(std::string("Compass Guard"), std::string("Compass Long"), na<double>(), ((signed_position_size() == 0.0 ? na<double>() : position_entry_price_) - (atrValue * stopAtr)), na<double>(), na<double>(), na<double>(), 100.0, "", na<double>(), "", na<double>(), na<double>());
        }
    }
