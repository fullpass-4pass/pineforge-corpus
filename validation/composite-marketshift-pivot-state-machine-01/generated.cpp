...
public:
    ...
    explicit GeneratedStrategy() : _ta_pivothigh_1(5, 5), _ta_pivotlow_2(5, 5), last_ph(na<double>()), last_pl(na<double>()), market_state(0) {
    ...
    void on_bar(const Bar& bar) override {
        ...
        if (((market_state == 1) && (signed_position_size() <= 0))) {
            strategy_entry(std::string("L"), true, na<double>(), na<double>(), 1, std::string("state bull"), "", 0, -1);
        }
        if (((market_state == (-1)) && (signed_position_size() >= 0))) {
            strategy_entry(std::string("S"), false, na<double>(), na<double>(), 1, std::string("state bear"), "", 0, -1);
        }
    }
