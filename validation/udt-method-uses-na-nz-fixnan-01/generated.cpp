...
struct Sample {
    double prev = na<double>();
    double ratio = na<double>();
...
public:
    ...
    double _udt_Sample_safeRatio(Sample& self, double curr) {
        double p = (is_na(self.prev) ? curr : self.prev);
        r = (((p == 0.0)) ? (1.0) : (((double)(curr) / (double)(p))));
        self.prev = curr;
        self.ratio = r;
        return ((is_na(self.ratio)) ? (1.0) : (self.ratio));
    }

    void on_bar(const Bar& bar) override {
        if (!_var_initialized) {
            s = Sample{.prev = na<double>(), .ratio = na<double>()};
            _var_initialized = true;
        } else {
        }
        r = _udt_Sample_safeRatio(s, current_bar_.close);
        ...
        entryCond = (((is_first_tick_ ? _ta_crossover_3.compute(emaFast, emaSlow) : _ta_crossover_3.recompute(emaFast, emaSlow)) && (r > 1.0)) && !(is_na(r)));
