...
public:
    ...
    Series<bool> stack_bear;
    Series<bool> stack_bull;
    int i_xs = 0;
    int i_s = 0;
    int i_m = 0;
    int i_l = 0;
    ...
    double ema_m = 0.0;
    double ema_l = 0.0;
    bool bull_edge = false;
    bool bear_edge = false;
    bool _ta_initialized_ = false;
    bool _inputs_initialized_ = false;

    explicit GeneratedStrategy() : _ta_ema_1(8), _ta_ema_2(21), _ta_ema_3(55), _ta_ema_4(200) {
    ...
    void on_bar(const Bar& bar) override {
        ...
        stack_bull.push((((ema_xs > ema_s) && (ema_s > ema_m)) && (ema_m > ema_l)));
        stack_bear.push((((ema_xs < ema_s) && (ema_s < ema_m)) && (ema_m < ema_l)));
        bull_edge = (stack_bull[0] && !(stack_bull[1]));
        bear_edge = (stack_bear[0] && !(stack_bear[1]));
        if ((bull_edge && (signed_position_size() <= 0))) {
            strategy_entry(std::string("L"), true, na<double>(), na<double>(), 1, std::string("stack bull edge"), "", 0, -1);
        }
        if ((bear_edge && (signed_position_size() >= 0))) {
