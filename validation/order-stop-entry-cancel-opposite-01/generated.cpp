...
public:
    ...
    explicit GeneratedStrategy() : _ta_ema_1(8), _ta_ema_2(21) {
    ...
    void on_bar(const Bar& bar) override {
        ...
        shortStop = _s_low[1];
        if ((fast > slow)) {
            strategy_entry(std::string("LE"), true, na<double>(), longStop, na<double>(), std::string("live long stop"), "", 0, -1);
            strategy_cancel(std::string("SE"));
        } else {
            strategy_entry(std::string("SE"), false, na<double>(), shortStop, na<double>(), std::string("live short stop"), "", 0, -1);
            strategy_cancel(std::string("LE"));
        }
        if (((signed_position_size() > 0) && (fast < slow))) {
