...
public:
    ...
    explicit GeneratedStrategy() : _ta_pivothigh_1(4, 4), _ta_pivotlow_2(4, 4), last_ph(na<double>()), last_pl(na<double>()) {
    ...
    void on_bar(const Bar& bar) override {
        ...
        if (!_ta_initialized_) {
            ...
            _ta_pivotlow_2 = ta::PivotLow(get_input_int("Pivot left bars", 4), get_input_int("Pivot right bars", 4));
        ...
        long_break = (!(is_na(last_ph)) && (current_bar_.close > last_ph));
        short_break = (!(is_na(last_pl)) && (current_bar_.close < last_pl));
        if ((long_break && (signed_position_size() <= 0))) {
            ...
            strategy_entry(std::string("L"), true, na<double>(), na<double>(), na<double>(), std::string("pivot-high break"));
        }
        if ((short_break && (signed_position_size() >= 0))) {
            if ((signed_position_size() > 0)) {
                strategy_close(std::string("L"), std::string("flip flat"), na<double>(), na<double>(), false);
            }
            strategy_entry(std::string("S"), false, na<double>(), na<double>(), na<double>(), std::string("pivot-low break"));
        }
    }
    ...
    void precalculate(const Bar* bars, int n) {
        ...
        _ta_pivothigh_1 = ta::PivotHigh(4, 4);
        _ta_pivotlow_2 = ta::PivotLow(4, 4);
