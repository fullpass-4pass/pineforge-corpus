...
public:
    ...
    explicit GeneratedStrategy() : _ta_pivothigh_1(10, 5), _ta_pivotlow_2(10, 5), last_swing_high(na<double>()), last_swing_low(na<double>()) {
    ...
    void on_bar(const Bar& bar) override {
        ...
        if (!_ta_initialized_) {
            _ta_pivothigh_1 = ta::PivotHigh(get_input_int("Swing Left Bars", 10), get_input_int("Swing Right Bars", 5));
            _ta_pivotlow_2 = ta::PivotLow(get_input_int("Swing Left Bars", 10), get_input_int("Swing Right Bars", 5));
            _ta_initialized_ = true;
        }
        swing_high = (is_first_tick_ ? _ta_pivothigh_1.compute(current_bar_.high) : _ta_pivothigh_1.recompute(current_bar_.high));
        swing_low = (is_first_tick_ ? _ta_pivotlow_2.compute(current_bar_.low) : _ta_pivotlow_2.recompute(current_bar_.low));
        if (!(is_na(swing_high))) {
        ...
        if ((sweep_low && (signed_position_size() <= 0))) {
            if ((signed_position_size() < 0)) {
                strategy_close(std::string("S"), std::string("flip"), na<double>(), na<double>(), false);
            }
            strategy_entry(std::string("L"), true, na<double>(), na<double>(), 1, std::string("sweep low → long"), "", 0, -1);
        }
        if ((sweep_high && (signed_position_size() >= 0))) {
            if ((signed_position_size() > 0)) {
                strategy_close(std::string("L"), std::string("flip"), na<double>(), na<double>(), false);
            }
            strategy_entry(std::string("S"), false, na<double>(), na<double>(), 1, std::string("sweep high → short"), "", 0, -1);
        }
    }
    ...
    void precalculate(const Bar* bars, int n) {
        ...
        _ta_pivothigh_1 = ta::PivotHigh(10, 5);
        _ta_pivotlow_2 = ta::PivotLow(10, 5);
