...
public:
    ...
    explicit GeneratedStrategy() : _ta_cci_1(20) {
    ...
    void on_bar(const Bar& bar) override {
        if (!_inputs_initialized_) {
            cciLen = get_input_int("CCI Length", 20);
            obLevel = get_input_int("Overbought", 100);
            osLevel = get_input_int("Oversold", (-100));
        ...
        cciVal = (is_first_tick_ ? _ta_cci_1.compute(current_bar_.close) : _ta_cci_1.recompute(current_bar_.close));
        longCond = (is_first_tick_ ? _ta_crossover_2.compute(cciVal, osLevel) : _ta_crossover_2.recompute(cciVal, osLevel));
        shortCond = (is_first_tick_ ? _ta_crossunder_3.compute(cciVal, obLevel) : _ta_crossunder_3.recompute(cciVal, obLevel));
        if (longCond) {
        ...
        if (((signed_position_size() > 0) && (is_first_tick_ ? _ta_crossunder_4.compute(cciVal, 0) : _ta_crossunder_4.recompute(cciVal, 0)))) {
            strategy_close(std::string("Long"), "", na<double>(), na<double>(), false);
        }
        if (((signed_position_size() < 0) && (is_first_tick_ ? _ta_crossover_5.compute(cciVal, 0) : _ta_crossover_5.recompute(cciVal, 0)))) {
