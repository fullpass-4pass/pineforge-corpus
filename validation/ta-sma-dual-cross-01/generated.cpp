...
public:
    ...
    void on_bar(const Bar& bar) override {
        if (!_inputs_initialized_) {
            i_fast = get_input_int("Fast SMA length", 9);
            i_slow = get_input_int("Slow SMA length", 21);
        ...
        fast_ma = (is_first_tick_ ? _ta_sma_1.compute(current_bar_.close) : _ta_sma_1.recompute(current_bar_.close));
        slow_ma = (is_first_tick_ ? _ta_sma_2.compute(current_bar_.close) : _ta_sma_2.recompute(current_bar_.close));
        golden_cross = (is_first_tick_ ? _ta_crossover_3.compute(fast_ma, slow_ma) : _ta_crossover_3.recompute(fast_ma, slow_ma));
        death_cross = (is_first_tick_ ? _ta_crossunder_4.compute(fast_ma, slow_ma) : _ta_crossunder_4.recompute(fast_ma, slow_ma));
        if ((golden_cross && (signed_position_size() <= 0))) {
            if ((signed_position_size() < 0)) {
                strategy_close(std::string("S"), std::string("flip flat"), na<double>(), na<double>(), false);
            }
            strategy_entry(std::string("L"), true, na<double>(), na<double>(), na<double>(), std::string("golden cross"));
        }
        if ((death_cross && (signed_position_size() >= 0))) {
            if ((signed_position_size() > 0)) {
                strategy_close(std::string("L"), std::string("flip flat"), na<double>(), na<double>(), false);
            }
            strategy_entry(std::string("S"), false, na<double>(), na<double>(), na<double>(), std::string("death cross"));
        }
    }
