...
public:
    ...
    double pivotLow = 0.0;
    bool pivotHighConfirmed = false;
    bool pivotLowConfirmed = false;
    bool _ta_initialized_ = false;
    bool _inputs_initialized_ = false;

    explicit GeneratedStrategy() : _ta_pivothigh_1(5, 5), _ta_pivotlow_2(5, 5) {
    ...
    void on_bar(const Bar& bar) override {
        ...
        pivotHighConfirmed = !(is_na(pivotHigh));
        pivotLowConfirmed = !(is_na(pivotLow));
        if ((pivotLowConfirmed && (signed_position_size() == 0))) {
            strategy_entry(std::string("L"), true, na<double>(), na<double>(), 1, std::string("pivot low entry"), "", 0, -1);
        }
        if ((pivotHighConfirmed && (signed_position_size() > 0))) {
            strategy_close(std::string("L"), std::string("pivot high exit"), na<double>(), na<double>(), false);
        }
    }
