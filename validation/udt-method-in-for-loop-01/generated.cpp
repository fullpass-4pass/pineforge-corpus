...
public:
    ...
    explicit GeneratedStrategy() : _ta_ema_1(21) {
    ...
    double _udt_Acc_add(Acc& self, double v) {
        self.total = (self.total + v);
        self.n = (self.n + 1);
        return self.total;
    }

    double _udt_Acc_mean(Acc& self) {
        return (((self.n > 0)) ? (((double)(self.total) / (double)((double)(self.n)))) : (0.0));
    }

    int _udt_Acc_reset(Acc& self) {
        self.total = 0.0;
        self.n = 0;
        return 0;
    }
    ...
    void on_bar(const Bar& bar) override {
        ...
        if (!_var_initialized) {
            acc = Acc{.total = 0.0, .n = 0};
            _var_initialized = true;
        } else {
        }
        _r = _udt_Acc_reset(acc);
        for (int i = 0; i <= 4; i += 1) {
            double _t = _udt_Acc_add(acc, _s_close[i]);
        }
        avg5 = _udt_Acc_mean(acc);
        emaSlow = (is_first_tick_ ? _ta_ema_1.compute(current_bar_.close) : _ta_ema_1.recompute(current_bar_.close));
        if ((((avg5 > emaSlow) && (is_first_tick_ ? _ta_crossover_2.compute(current_bar_.close, emaSlow) : _ta_crossover_2.recompute(current_bar_.close, emaSlow))) && (signed_position_size() == 0))) {
            strategy_entry(std::string("L"), true, na<double>(), na<double>(), 1, std::string("entry long"), "", 0, -1);
        }
        if (((is_first_tick_ ? _ta_crossunder_3.compute(current_bar_.close, emaSlow) : _ta_crossunder_3.recompute(current_bar_.close, emaSlow)) && (signed_position_size() > 0))) {
