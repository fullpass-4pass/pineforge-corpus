...
public:
    ...
    struct _PFScriptState {
        ...
        decltype(GeneratedStrategy::_ta_atr_3) _pf_value_4;
        decltype(GeneratedStrategy::_ta_crossover_4) _pf_value_5;
        decltype(GeneratedStrategy::_ta_crossunder_5) _pf_value_6;
        decltype(GeneratedStrategy::_sec0__ta_ema_1) _pf_value_7;
        decltype(GeneratedStrategy::higherTimeframe) _pf_value_8;
        decltype(GeneratedStrategy::higherLength) _pf_value_9;
        decltype(GeneratedStrategy::localLength) _pf_value_10;
        ...
        decltype(GeneratedStrategy::localCenter) _pf_value_14;
    ...
    void restore_script_state() override {
        ...
        this->_ta_atr_3 = _pf_script_state_checkpoint_->_pf_value_4;
        ...
        this->_sec0__ta_ema_1 = _pf_script_state_checkpoint_->_pf_value_7;
        this->higherTimeframe = _pf_script_state_checkpoint_->_pf_value_8;
        this->higherLength = _pf_script_state_checkpoint_->_pf_value_9;
        this->localLength = _pf_script_state_checkpoint_->_pf_value_10;
        ...
        this->localCenter = _pf_script_state_checkpoint_->_pf_value_14;
    ...
    explicit GeneratedStrategy() : _ta_ema_1(34), _ta_ema_2(13), _ta_atr_3(16), _sec0__ta_ema_1(34) {
    ...
    void on_bar(const Bar& bar) override {
        if (!_inputs_initialized_) {
            higherTimeframe = get_input_string("Higher Timeframe", std::string("60"));
            higherLength = get_input_int("Higher EMA", 34);
            localLength = get_input_int("Local EMA", 13);
            atrLength = get_input_int("ATR Length", 16);
            stopAtr = get_input_double("Stop ATR", 2.2);
            _inputs_initialized_ = true;
        }
        if (!_ta_initialized_) {
            _ta_ema_1 = ta::EMA(get_input_int("Higher EMA", 34));
            _ta_ema_2 = ta::EMA(get_input_int("Local EMA", 13));
            _ta_atr_3 = ta::ATR(get_input_int("ATR Length", 16));
            _sec0__ta_ema_1 = ta::EMA(get_input_int("Higher EMA", 34));
            _ta_initialized_ = true;
        }
        higherCenter = _req_sec_0;
        localCenter = (history_advances_new_bar() ? _ta_ema_2.compute(current_bar_.close) : _ta_ema_2.recompute(current_bar_.close));
        ...
        if ((history_advances_new_bar() ? _ta_crossover_4.compute(localCenter, higherCenter) : _ta_crossover_4.recompute(localCenter, higherCenter))) {
            strategy_entry(std::string("Orbit Long"), true, na<double>(), na<double>(), na<double>(), "");
        }
        if ((([&]{ auto _pna_l = (signed_position_size()); auto _pna_r = (0); double _pfc_l = static_cast<double>(_pna_l); double _pfc_r = static_cast<double>(_pna_r); bool _pfc_eq = (_pfc_l == _pfc_r) || (std::isfinite(_pfc_l) && std::isfinite(_pfc_r) && std::fabs(_pfc_l - _pfc_r) <= 1e-10); return !is_na(_pna_l) && !is_na(_pna_r) && ((_pfc_l > _pfc_r) && !_pfc_eq); }()) && (history_advances_new_bar() ? _ta_crossunder_5.compute(localCenter, higherCenter) : _ta_crossunder_5.recompute(localCenter, higherCenter)))) {
            strategy_close(std::string("Orbit Long"), std::string("Higher-timeframe trend changed"), na<double>(), na<double>(), false, 124554051603ULL);
        }
        if (([&]{ auto _pna_l = (signed_position_size()); auto _pna_r = (0); double _pfc_l = static_cast<double>(_pna_l); double _pfc_r = static_cast<double>(_pna_r); bool _pfc_eq = (_pfc_l == _pfc_r) || (std::isfinite(_pfc_l) && std::isfinite(_pfc_r) && std::fabs(_pfc_l - _pfc_r) <= 1e-10); return !is_na(_pna_l) && !is_na(_pna_r) && ((_pfc_l > _pfc_r) && !_pfc_eq); }())) {
            strategy_exit(std::string("Orbit Guard"), std::string("Orbit Long"), na<double>(), ((signed_position_size() == 0.0 ? na<double>() : position_entry_price_) - (atrValue * stopAtr)), na<double>(), na<double>(), na<double>(), 100.0, "", na<double>(), "", na<double>(), na<double>());
        }
    }
