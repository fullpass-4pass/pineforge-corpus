...
public:
    ...
    ta::MACD _ta_macd_2;
    std::vector<ta::MACDResult> _precalc__ta_macd_2;
    bool _use_precalc = false;
    std::string rules = std::string("");
    std::string lrules = std::string("");
    bool useRsi = false;
    bool useMacd = false;
    double rsiVal = 0.0;
    bool longCond = false;
    ...
    explicit GeneratedStrategy() : _ta_rsi_1(14), _ta_macd_2(12, 26, 9) {
    ...
    void on_bar(const Bar& bar) override {
        ...
        if ((pine_str_match(lrules, std::string("macd=long")) != std::string(""))) {
            useMacd = true;
        }
        rsiVal = (is_first_tick_ ? _ta_rsi_1.compute(current_bar_.close) : _ta_rsi_1.recompute(current_bar_.close));
        auto _result__ta_macd_2 = (is_first_tick_ ? _ta_macd_2.compute(current_bar_.close) : _ta_macd_2.recompute(current_bar_.close));
        double macdLine = _result__ta_macd_2.macd_line;
        double sigLine = _result__ta_macd_2.signal_line;
        longCond = true;
        if ((useRsi && (rsiVal <= 50))) {
            longCond = false;
        }
        if ((useMacd && (macdLine <= sigLine))) {
            longCond = false;
        }
        if (longCond) {
    ...
    void precalculate(const Bar* bars, int n) {
        ...
        _precalc__ta_macd_2.resize(n);

        _ta_rsi_1 = ta::RSI(14);
        _ta_macd_2 = ta::MACD(12, 26, 9);


        for (int i = 0; i < n; ++i) {
            _precalc__ta_rsi_1[i] = _ta_rsi_1.compute(bars[i].close);
            _precalc__ta_macd_2[i] = _ta_macd_2.compute(bars[i].close);
        }
