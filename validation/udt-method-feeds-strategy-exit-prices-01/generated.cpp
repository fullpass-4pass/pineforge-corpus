...
struct Bracket {
    double entry_px = na<double>();
    double entry_atr = na<double>();
...
public:
    ...
    double _udt_Bracket_stopPrice(Bracket& self) {
        return (self.entry_px - (self.entry_atr * self.stop_mult));
    }

    double _udt_Bracket_limitPrice(Bracket& self) {
        return (self.entry_px + (self.entry_atr * self.limit_mult));
    }

    void on_bar(const Bar& bar) override {
        if (!_var_initialized) {
            br = Bracket{.entry_px = na<double>(), .entry_atr = na<double>(), .stop_mult = 1.5, .limit_mult = 3.0};
