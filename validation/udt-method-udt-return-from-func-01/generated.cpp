...
public:
    ...
    double _udt_Sample_score(Sample& self) {
        ...
        if ((is_na(self.price) || is_na(self.rsi))) {
            ...
            _func_ret = (((self.rsi - 50.0) * 0.1) * self.weight);
        }
        return _func_ret;
    }

    Sample build_sample(double price, double rsi, double weight) {
        return Sample{.price = price, .rsi = rsi, .weight = weight};
    }
    ...
    void on_bar(const Bar& bar) override {
        ...
        s = build_sample(current_bar_.close, rsiVal, 1.0);
        sc = _udt_Sample_score(s);
        if ((((is_first_tick_ ? _ta_crossover_4.compute(emaFast, emaSlow) : _ta_crossover_4.recompute(emaFast, emaSlow)) && (sc > 1.0)) && (signed_position_size() == 0))) {
