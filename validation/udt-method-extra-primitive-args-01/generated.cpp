...
public:
    ...
    double _udt_Cfg_offset(Cfg& self, int n, double mult, bool flip) {
        int sign = ((flip) ? ((-1)) : (1));
        return ((self.base + ((mult * (double)(n)) * (double)(sign))) + self.bias);
    }

    void on_bar(const Bar& bar) override {
        if (!_var_initialized) {
            cfg = Cfg{.base = 0.0, .bias = 0.0};
            _var_initialized = true;
        } else {
        }
        cfg.base = current_bar_.close;
        upOff = _udt_Cfg_offset(cfg, 5, 0.1, false);
