...
public:
    std::vector<bool> _req_sec_lower_tf_0{};
    ...
    void on_bar(const Bar& bar) override {
        ltfBullArr = _req_sec_lower_tf_0;
        bullCount = 0;
        if (((double)ltfBullArr.size() > 0)) {
            for (int i = 0; i <= ((double)ltfBullArr.size() - 1); i += 1) {
                if (ltfBullArr[i]) {
        ...
        entryCond = ((bullCount >= 9) && (emaFast > emaSlow));
        exitCond = ((bullCount <= 6) || (emaFast < emaSlow));
        if ((entryCond && (signed_position_size() == 0))) {
            strategy_entry(std::string("L"), true, na<double>(), na<double>(), 1, std::string("bull-majority"), "", 0, -1);
        }
        if ((exitCond && (signed_position_size() > 0))) {
            strategy_close(std::string("L"), std::string("bull-broken"), na<double>(), na<double>(), false);
        }
    }
    ...
    void _eval_security_0(const Bar& bar, bool is_complete) {
        ...
        _req_sec_lower_tf_0.push_back((bar.close > bar.open));
    }
