...
struct Store {
    ...
    bool __pf_na = true;
    static Store create() { return Store{}; }
};
inline bool is_na(const Store& _z) { return _z.__pf_na; }
...
public:
    ...
    void on_bar(const Bar& bar) override {
        if (!_var_initialized) {
            s = Store{.lines = std::vector<Line>(), .__pf_na = false};
            _var_initialized = true;
        } else {
        }
        target = (is_first_tick_ ? _ta_sma_1.compute(current_bar_.close) : _ta_sma_1.recompute(current_bar_.close));
        if (is_na(alias)) {
            l = pf_line_new(_pf_lines_, (int64_t)(bar_index_), (double)(target), (int64_t)(bar_index_), (double)(target), XLoc::bar_index, false, false);
            s.lines.push_back(l);
            alias = l;
        }
        lvl = na<double>();
        if (((double)s.lines.size() > 0)) {
            e = s.lines[(0)];
            pf_line_set_y2(_pf_lines_, e, (double)(target));
            lvl = pf_line_get_y2(_pf_lines_, alias);
        }
        ref = (is_na(lvl) ? current_bar_.close : lvl);
        ready = !(is_na(lvl));
        eUp = (is_first_tick_ ? _ta_crossover_2.compute(current_bar_.close, ref) : _ta_crossover_2.recompute(current_bar_.close, ref));
        eDn = (is_first_tick_ ? _ta_crossunder_3.compute(current_bar_.close, ref) : _ta_crossunder_3.recompute(current_bar_.close, ref));
        if ((ready && eUp)) {
            strategy_entry(std::string("L"), true, na<double>(), na<double>(), na<double>(), "");
        }
        if ((((signed_position_size() > 0) && ready) && eDn)) {
