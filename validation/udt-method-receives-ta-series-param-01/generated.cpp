...
struct Filter {
    ...
    double rsi_max = 90.0;
    double atr_min = 0.0;
    static Filter create() { return Filter{}; }
};
...
public:
    ...
    explicit GeneratedStrategy() : _ta_rsi_1(14), _ta_ema_2(21), _ta_atr_3(14) {
    ...
    bool _udt_Filter_confirms(Filter& self, double rsi, double ema, double atr) {
        bool rsiOk = ((rsi >= self.rsi_min) && (rsi <= self.rsi_max));
        bool emaOk = (current_bar_.close > ema);
        bool atrOk = (atr > self.atr_min);
        return ((rsiOk && emaOk) && atrOk);
    }

    void on_bar(const Bar& bar) override {
        if (!_var_initialized) {
            filt = Filter{.rsi_min = 40.0, .rsi_max = 90.0, .atr_min = 0.0};
        ...
        xUp = (is_first_tick_ ? _ta_crossover_4.compute(current_bar_.close, emaVal) : _ta_crossover_4.recompute(current_bar_.close, emaVal));
        xDown = (is_first_tick_ ? _ta_crossunder_5.compute(current_bar_.close, emaVal) : _ta_crossunder_5.recompute(current_bar_.close, emaVal));
        entryCond = (xUp && _udt_Filter_confirms(filt, rsiVal, emaVal, atrVal));
        exitCond = xDown;
