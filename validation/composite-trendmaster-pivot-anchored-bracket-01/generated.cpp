...
public:
    ...
    explicit GeneratedStrategy() : _ta_pivothigh_1(5, 5), _ta_pivotlow_2(5, 5), _ta_ema_3(5), _ta_ema_4(13), last_ph(na<double>()), last_pl(na<double>()) {
    ...
    void on_bar(const Bar& bar) override {
        ...
        ema_slow = (is_first_tick_ ? _ta_ema_4.compute(current_bar_.close) : _ta_ema_4.recompute(current_bar_.close));
        long_signal = (is_first_tick_ ? _ta_crossover_5.compute(ema_fast, ema_slow) : _ta_crossover_5.recompute(ema_fast, ema_slow));
        short_signal = (is_first_tick_ ? _ta_crossunder_6.compute(ema_fast, ema_slow) : _ta_crossunder_6.recompute(ema_fast, ema_slow));
        if (((long_signal && !(is_na(last_pl))) && (signed_position_size() <= 0))) {
            strategy_entry(std::string("L"), true, na<double>(), na<double>(), 1, std::string("pivot long"), "", 0, -1);
        }
        if (((short_signal && !(is_na(last_ph))) && (signed_position_size() >= 0))) {
            strategy_entry(std::string("S"), false, na<double>(), na<double>(), 1, std::string("pivot short"), "", 0, -1);
        }
