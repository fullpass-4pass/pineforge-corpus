...
public:
    ...
    explicit GeneratedStrategy() : _ta_sma_1(10), _ta_sma_2(20), _ta_sma_3(50), prevBull(false), prevBear(false) {
    ...
    void on_bar(const Bar& bar) override {
        ...
        ma1 = (is_first_tick_ ? _ta_sma_1.compute(current_bar_.close) : _ta_sma_1.recompute(current_bar_.close));
        ma2 = (is_first_tick_ ? _ta_sma_2.compute(current_bar_.close) : _ta_sma_2.recompute(current_bar_.close));
        ma3 = (is_first_tick_ ? _ta_sma_3.compute(current_bar_.close) : _ta_sma_3.recompute(current_bar_.close));
        bullStack = ((ma1 > ma2) && (ma2 > ma3));
        bearStack = ((ma1 < ma2) && (ma2 < ma3));
        if ((bullStack && !(prevBull))) {
            strategy_entry(std::string("Long"), true, na<double>(), na<double>(), na<double>(), "");
        }
        if ((bearStack && !(prevBear))) {
            strategy_entry(std::string("Short"), false, na<double>(), na<double>(), na<double>(), "");
        }
        prevBull = bullStack;
        prevBear = bearStack;
        if (((signed_position_size() > 0) && !(bullStack))) {
            strategy_close(std::string("Long"), "", na<double>(), na<double>(), false);
        }
        if (((signed_position_size() < 0) && !(bearStack))) {
