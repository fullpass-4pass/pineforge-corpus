...
public:
    ...
    explicit GeneratedStrategy() : _ta_ema_1(9), _ta_ema_2(21), pendingLong(false), pendingExit(false) {
    ...
    void on_bar(const Bar& bar) override {
        ...
        if ((is_first_tick_ ? _ta_crossover_3.compute(emaFast, emaSlow) : _ta_crossover_3.recompute(emaFast, emaSlow))) {
            pendingLong = true;
        }
        if ((is_first_tick_ ? _ta_crossunder_4.compute(emaFast, emaSlow) : _ta_crossunder_4.recompute(emaFast, emaSlow))) {
            pendingExit = true;
        }
        if (((is_last_tick_ && pendingLong) && (signed_position_size() == 0))) {
            strategy_entry(std::string("L"), true, na<double>(), na<double>(), 1, std::string("confirmed long"), "", 0, -1);
            pendingLong = false;
        }
        if (((is_last_tick_ && pendingExit) && (signed_position_size() > 0))) {
            strategy_close(std::string("L"), std::string("confirmed exit"), na<double>(), na<double>(), false);
            pendingExit = false;
        }
    }
