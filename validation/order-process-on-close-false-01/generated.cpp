...
public:
    ...
    void on_bar(const Bar& bar) override {
        ...
        entryCond = ((is_first_tick_ ? _ta_crossover_4.compute(emaFast, emaSlow) : _ta_crossover_4.recompute(emaFast, emaSlow)) && (rsiVal < 70));
        exitCond = (is_first_tick_ ? _ta_crossunder_5.compute(emaFast, emaSlow) : _ta_crossunder_5.recompute(emaFast, emaSlow));
        ...
        if ((exitCond && (signed_position_size() > 0))) {
            strategy_close(std::string("L"), std::string("exit"), na<double>(), na<double>(), false);
        }
    }
