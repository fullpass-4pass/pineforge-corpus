...
public:
    ...
    std::vector<ta::SupertrendResult> _precalc__ta_supertrend_1;
    ...
    explicit GeneratedStrategy() : _ta_supertrend_1(3.0, 10), prev_dir(0) {
    ...
    void on_bar(const Bar& bar) override {
        ...
        double st_trail = _result__ta_supertrend_1.value;
        double st_dir = _result__ta_supertrend_1.direction;
        ...
        if ((bull_flip && (signed_position_size() <= 0))) {
            ...
            strategy_entry(std::string("L"), true, na<double>(), na<double>(), na<double>(), std::string("st bull flip"));
        }
        if ((bear_flip && (signed_position_size() >= 0))) {
            if ((signed_position_size() > 0)) {
                strategy_close(std::string("L"), std::string("flip flat"), na<double>(), na<double>(), false);
            }
            strategy_entry(std::string("S"), false, na<double>(), na<double>(), na<double>(), std::string("st bear flip"));
        }
        prev_dir = st_dir;
    }
