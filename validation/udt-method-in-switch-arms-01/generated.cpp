...
struct Counters {
    ...
    int bear_run = 0;
    int flat_run = 0;
    static Counters create() { return Counters{}; }
};
...
public:
    ...
    int _udt_Counters_bumpBull(Counters& self) {
    ...
    int _udt_Counters_bumpBear(Counters& self) {
    ...
    int _udt_Counters_bumpFlat(Counters& self) {
    ...
    void on_bar(const Bar& bar) override {
        if (!_var_initialized) {
            c = Counters{.bull_run = 0, .bear_run = 0, .flat_run = 0};
        ...
        regime = (((emaFast > (emaSlow * 1.001))) ? (std::string("bull")) : ((((emaFast < (emaSlow * 0.999))) ? (std::string("bear")) : (std::string("flat")))));
        auto __switch_val_0 = regime;
        if (__switch_val_0 == std::string("bull")) {
            active = _udt_Counters_bumpBull(c);
        }
        else if (__switch_val_0 == std::string("bear")) {
            active = _udt_Counters_bumpBear(c);
        }
        else {
            active = _udt_Counters_bumpFlat(c);
        }
        if ((((regime == std::string("bull")) && (c.bull_run >= 3)) && (signed_position_size() == 0))) {
            strategy_entry(std::string("L"), true, na<double>(), na<double>(), 1, std::string("entry long"), "", 0, -1);
        }
        if ((((regime == std::string("bear")) && (c.bear_run >= 3)) && (signed_position_size() > 0))) {
