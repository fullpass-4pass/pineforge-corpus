...
public:
    ...
    void on_bar(const Bar& bar) override {
        ...
        entryCond = (is_first_tick_ ? _ta_crossover_2.compute(rsiVal, 50.0) : _ta_crossover_2.recompute(rsiVal, 50.0));
        exitCond = (is_first_tick_ ? _ta_crossunder_3.compute(rsiVal, 50.0) : _ta_crossunder_3.recompute(rsiVal, 50.0));
        if ((entryCond && (signed_position_size() == 0))) {
            strategy_entry(std::string("L"), true, na<double>(), na<double>(), 1, std::string("entry long"), "", 0, -1);
            stopLvl = ((current_bar_.open + current_bar_.high) * 0.5);
            strategy_exit(std::string("X"), std::string("L"), na<double>(), stopLvl, na<double>(), na<double>(), na<double>(), 100.0, std::string("mid-bar stop"), na<double>(), "");
        }
