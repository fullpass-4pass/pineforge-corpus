...
public:
    ...
    ta::Lowest _ta_lowest_3;
    std::vector<double> _precalc__ta_lowest_3;
    bool _use_precalc = false;
    Series<int> direction;
    Series<double> chandLong;
    Series<double> chandShort;
    int atrLen = 0;
    double atrMult = 0.0;
    int lookback = 0;
    double atrVal = 0.0;
    ...
    explicit GeneratedStrategy() : _ta_atr_1(22), _ta_highest_2(22), _ta_lowest_3(22) {
    ...
    void on_bar(const Bar& bar) override {
        if (!_var_initialized) {
            ...
            if (is_first_tick_) direction.push(direction[0]);
        }
        ...
        highestHigh = (is_first_tick_ ? _ta_highest_2.compute(current_bar_.high) : _ta_highest_2.recompute(current_bar_.high));
        lowestLow = (is_first_tick_ ? _ta_lowest_3.compute(current_bar_.low) : _ta_lowest_3.recompute(current_bar_.low));
        chandLong.push((highestHigh - (atrVal * atrMult)));
        chandShort.push((lowestLow + (atrVal * atrMult)));
        if ((current_bar_.close > chandShort[1])) {
            direction.update(1);
        }
        if ((current_bar_.close < chandLong[1])) {
            direction.update((-1));
        }
        longCond = ((direction[0] == 1) && (direction[1] != 1));
        shortCond = ((direction[0] == (-1)) && (direction[1] != (-1)));
        if (longCond) {
