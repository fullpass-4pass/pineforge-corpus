...
public:
    ...
    void on_bar(const Bar& bar) override {
        if (!_inputs_initialized_) {
            ...
            i_stop_x = get_input_double("Stop = N x ATR", 1.0);
            i_tp_x = get_input_double("TP   = N x ATR", 2.0);
        ...
        atr_val = (is_first_tick_ ? _ta_atr_1.compute(current_bar_.high, current_bar_.low, current_bar_.close) : _ta_atr_1.recompute(current_bar_.high, current_bar_.low, current_bar_.close));
        ...
        fire = ((at_entry_window && (signed_position_size() == 0)) && !(is_na(atr_val)));
        if (fire) {
