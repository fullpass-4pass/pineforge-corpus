...
public:
    ...
    explicit GeneratedStrategy() : _ta_pivothigh_1(5, 5), _ta_pivotlow_2(5, 5), last_ph(na<double>()), last_pl(na<double>()), market_state(0), prev_state(0) {
    ...
    void on_bar(const Bar& bar) override {
        ...
        break_up = (!(is_na(last_ph)) && (current_bar_.close > last_ph));
        break_down = (!(is_na(last_pl)) && (current_bar_.close < last_pl));
        if (break_up) {
            market_state = 1;
        } else
        if (break_down) {
        ...
        long_edge = ((market_state == 1) && (prev_state != 1));
        short_edge = ((market_state == (-1)) && (prev_state != (-1)));
        if ((long_edge && (signed_position_size() <= 0))) {
            strategy_entry(std::string("L"), true, na<double>(), na<double>(), 1, std::string("bull edge"), "", 0, -1);
        }
        if ((short_edge && (signed_position_size() >= 0))) {
            strategy_entry(std::string("S"), false, na<double>(), na<double>(), 1, std::string("bear edge"), "", 0, -1);
        }
