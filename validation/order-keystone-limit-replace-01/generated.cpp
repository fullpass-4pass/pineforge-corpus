...
public:
    ...
    struct _PFScriptState {
        ...
        decltype(GeneratedStrategy::trend) _pf_value_2;
        decltype(GeneratedStrategy::trendLength) _pf_value_3;
        decltype(GeneratedStrategy::atrLength) _pf_value_4;
        decltype(GeneratedStrategy::pullbackAtr) _pf_value_5;
        decltype(GeneratedStrategy::stopAtr) _pf_value_6;
        decltype(GeneratedStrategy::targetAtr) _pf_value_7;
        decltype(GeneratedStrategy::atrValue) _pf_value_8;
        decltype(GeneratedStrategy::risingTrend) _pf_value_9;
        decltype(GeneratedStrategy::workingLimit) _pf_value_10;
    ...
    void restore_script_state() override {
        ...
        this->trend = _pf_script_state_checkpoint_->_pf_value_2;
        this->trendLength = _pf_script_state_checkpoint_->_pf_value_3;
        this->atrLength = _pf_script_state_checkpoint_->_pf_value_4;
        this->pullbackAtr = _pf_script_state_checkpoint_->_pf_value_5;
        this->stopAtr = _pf_script_state_checkpoint_->_pf_value_6;
        this->targetAtr = _pf_script_state_checkpoint_->_pf_value_7;
        this->atrValue = _pf_script_state_checkpoint_->_pf_value_8;
        this->risingTrend = _pf_script_state_checkpoint_->_pf_value_9;
        this->workingLimit = _pf_script_state_checkpoint_->_pf_value_10;
    ...
    explicit GeneratedStrategy() : _ta_ema_1(48), _ta_atr_2(18) {
    ...
    void on_bar(const Bar& bar) override {
        if (!_inputs_initialized_) {
            ...
            pullbackAtr = get_input_double("Pullback ATR", 0.55);
            stopAtr = get_input_double("Stop ATR", 2.3);
            targetAtr = get_input_double("Target ATR", 2.0);
            _inputs_initialized_ = true;
        }
        if (!_ta_initialized_) {
            _ta_ema_1 = ta::EMA(get_input_int("Trend EMA", 48));
        ...
        workingLimit = (trend[0] - (atrValue * pullbackAtr));
        if ((([&]{ auto _pna_l = (signed_position_size()); auto _pna_r = (0); double _pfc_l = static_cast<double>(_pna_l); double _pfc_r = static_cast<double>(_pna_r); bool _pfc_eq = (_pfc_l == _pfc_r) || (std::isfinite(_pfc_l) && std::isfinite(_pfc_r) && std::fabs(_pfc_l - _pfc_r) <= 1e-10); return !is_na(_pna_l) && !is_na(_pna_r) && (_pfc_eq); }()) && risingTrend)) {
            strategy_entry(std::string("Keystone Pullback"), true, workingLimit, na<double>(), na<double>(), "", "", 0, -1);
        } else
        if (([&]{ auto _pna_l = (signed_position_size()); auto _pna_r = (0); double _pfc_l = static_cast<double>(_pna_l); double _pfc_r = static_cast<double>(_pna_r); bool _pfc_eq = (_pfc_l == _pfc_r) || (std::isfinite(_pfc_l) && std::isfinite(_pfc_r) && std::fabs(_pfc_l - _pfc_r) <= 1e-10); return !is_na(_pna_l) && !is_na(_pna_r) && (_pfc_eq); }())) {
            strategy_cancel(std::string("Keystone Pullback"));
        }
        if (([&]{ auto _pna_l = (signed_position_size()); auto _pna_r = (0); double _pfc_l = static_cast<double>(_pna_l); double _pfc_r = static_cast<double>(_pna_r); bool _pfc_eq = (_pfc_l == _pfc_r) || (std::isfinite(_pfc_l) && std::isfinite(_pfc_r) && std::fabs(_pfc_l - _pfc_r) <= 1e-10); return !is_na(_pna_l) && !is_na(_pna_r) && ((_pfc_l > _pfc_r) && !_pfc_eq); }())) {
            strategy_exit(std::string("Keystone Risk"), std::string("Keystone Pullback"), ((signed_position_size() == 0.0 ? na<double>() : position_entry_price_) + (atrValue * targetAtr)), ((signed_position_size() == 0.0 ? na<double>() : position_entry_price_) - (atrValue * stopAtr)), na<double>(), na<double>(), na<double>(), 100.0, "", na<double>(), "", na<double>(), na<double>());
        }
    }
