...
public:
    ...
    void on_bar(const Bar& bar) override {
        ...
        cond = (rsi > 50.0);
        if (cond) {
            strategy_entry(std::string("L"), true, na<double>(), na<double>(), na<double>(), std::string("rsi > 50"));
        } else {
            strategy_close(std::string("L"), std::string("rsi <= 50"), na<double>(), na<double>(), false);
        }
    }
