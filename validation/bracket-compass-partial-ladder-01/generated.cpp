...
public:
    ...
    int atrLength = 0;
    double stopAtr = 0.0;
    double firstTargetAtr = 0.0;
    double secondTargetAtr = 0.0;
    double firstPercent = 0.0;
    double fastEma = 0.0;
    double slowEma = 0.0;
    double atrValue = 0.0;
    double sharedStop = 0.0;
    ...
    struct _PFScriptState {
        ...
        decltype(GeneratedStrategy::atrLength) _pf_value_6;
        decltype(GeneratedStrategy::stopAtr) _pf_value_7;
        decltype(GeneratedStrategy::firstTargetAtr) _pf_value_8;
        decltype(GeneratedStrategy::secondTargetAtr) _pf_value_9;
        decltype(GeneratedStrategy::firstPercent) _pf_value_10;
        ...
        decltype(GeneratedStrategy::sharedStop) _pf_value_14;
    ...
    void restore_script_state() override {
        ...
        this->atrLength = _pf_script_state_checkpoint_->_pf_value_6;
        this->stopAtr = _pf_script_state_checkpoint_->_pf_value_7;
        this->firstTargetAtr = _pf_script_state_checkpoint_->_pf_value_8;
        this->secondTargetAtr = _pf_script_state_checkpoint_->_pf_value_9;
        this->firstPercent = _pf_script_state_checkpoint_->_pf_value_10;
        ...
        this->sharedStop = _pf_script_state_checkpoint_->_pf_value_14;
    ...
    explicit GeneratedStrategy() : _ta_ema_1(15), _ta_ema_2(49), _ta_atr_3(17) {
    ...
    void on_bar(const Bar& bar) override {
        if (!_inputs_initialized_) {
            fastLength = get_input_int("Fast EMA", 15);
            slowLength = get_input_int("Slow EMA", 49);
            atrLength = get_input_int("ATR Length", 17);
            stopAtr = get_input_double("Shared Stop ATR", 1.9);
            firstTargetAtr = get_input_double("First Target ATR", 1.4);
            secondTargetAtr = get_input_double("Second Target ATR", 3.0);
            firstPercent = get_input_double("First Exit Percent", 40.0);
        ...
        if ((history_advances_new_bar() ? _ta_crossover_4.compute(fastEma, slowEma) : _ta_crossover_4.recompute(fastEma, slowEma))) {
            strategy_entry(std::string("Compass Ladder Long"), true, na<double>(), na<double>(), na<double>(), "");
        }
        if (([&]{ auto _pna_l = (signed_position_size()); auto _pna_r = (0); double _pfc_l = static_cast<double>(_pna_l); double _pfc_r = static_cast<double>(_pna_r); bool _pfc_eq = (_pfc_l == _pfc_r) || (std::isfinite(_pfc_l) && std::isfinite(_pfc_r) && std::fabs(_pfc_l - _pfc_r) <= 1e-10); return !is_na(_pna_l) && !is_na(_pna_r) && ((_pfc_l > _pfc_r) && !_pfc_eq); }())) {
            sharedStop = ((signed_position_size() == 0.0 ? na<double>() : position_entry_price_) - (atrValue * stopAtr));
            strategy_exit(std::string("Compass First"), std::string("Compass Ladder Long"), ((signed_position_size() == 0.0 ? na<double>() : position_entry_price_) + (atrValue * firstTargetAtr)), sharedStop, na<double>(), na<double>(), na<double>(), firstPercent, "", na<double>(), "", na<double>(), na<double>());
            strategy_exit(std::string("Compass Remainder"), std::string("Compass Ladder Long"), ((signed_position_size() == 0.0 ? na<double>() : position_entry_price_) + (atrValue * secondTargetAtr)), sharedStop, na<double>(), na<double>(), na<double>(), (100.0 - firstPercent), "", na<double>(), "", na<double>(), na<double>());
        }
    }
