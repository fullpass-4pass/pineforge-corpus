...
public:
    ...
    void on_bar(const Bar& bar) override {
        ...
        if (arm_long) {
            strategy_entry(std::string("LongOnRng"), true, na<double>(), _s_high[1], na<double>(), "", "", 0, -1);
        } else {
            strategy_cancel(std::string("LongOnRng"));
        }
        if (arm_short) {
            strategy_entry(std::string("ShortOnRng"), false, na<double>(), _s_low[1], na<double>(), "", "", 0, -1);
        } else {
            strategy_cancel(std::string("ShortOnRng"));
        }
    }
