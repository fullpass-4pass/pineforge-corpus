...
public:
    ...
    void on_bar(const Bar& bar) override {
        ...
        fast = (is_first_tick_ ? _ta_sma_1.compute(src) : _ta_sma_1.recompute(src));
        slow = (is_first_tick_ ? _ta_sma_2.compute(src) : _ta_sma_2.recompute(src));
        if ((is_first_tick_ ? _ta_crossover_3.compute(fast, slow) : _ta_crossover_3.recompute(fast, slow))) {
            strategy_entry(std::string("Long"), true, na<double>(), na<double>(), na<double>(), "");
        }
        if ((is_first_tick_ ? _ta_crossunder_4.compute(fast, slow) : _ta_crossunder_4.recompute(fast, slow))) {
    ...
    void precalculate(const Bar* bars, int n) {
        ...
        for (int i = 0; i < n; ++i) {
            _precalc__ta_sma_1[i] = _ta_sma_1.compute(src);
